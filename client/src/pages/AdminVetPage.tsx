...
export default function AdminVetPage() {
  ...
  async function load() {
    setLoading(true);
    try { setWeights((await utils.vet.adminListWeights.fetch()) as WeightRow[]); }
      ...
      <AdminLayout title="VET ADM" subtitle="Pesos editoriais e estatísticas coletivas do motor estratégico.">
        <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
          <Card className="space-y-4 p-6">
            <h2 className="text-lg font-bold text-slate-900">Configurar peso editorial</h2>
            <p className="text-sm text-slate-600">Opcional, de 0 a 10. Sem configuração, o VET usa peso padrão e mantém a análise histórica automática.</p>
            {(["exam", "subject", "conteudo", "weight"] as const).map(key => (
              <label className="block" key={key}>
                <span className="text-xs font-bold uppercase text-slate-500">{key === "exam" ? "Prova" : key === "subject" ? "Disciplina" : key === "conteudo" ? "Conteúdo" : "Peso (0–10)"}</span>
                <input type={key === "weight" ? "number" : "text"} min={key === "weight" ? 0 : undefined} max={key === "weight" ? 10 : undefined} step={key === "weight" ? .5 : undefined} value={form[key]} onChange={event => setForm(current => ({ ...current, [key]: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
              </label>
            ))}
          ...
          <Card className="p-6">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div><h2 className="text-lg font-bold">Pesos configurados</h2><p className="text-sm text-slate-500">{weights.length} ajuste(s)</p></div>
              <Button variant="outline" onClick={async () => { const result = await refreshStats.mutateAsync(); setMessage(`${result.updatedGroups} grupo(s) coletivo(s) atualizado(s).`); }} disabled={refreshStats.isPending}><RefreshCw className="mr-2 h-4 w-4" />Atualizar média coletiva</Button>
            </div>
