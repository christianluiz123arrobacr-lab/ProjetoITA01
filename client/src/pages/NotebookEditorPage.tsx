...
export default function NotebookEditorPage() {
  ...
  async function savePdfToDrive() {
    ...
    try { const result = await exportPdf.mutateAsync({ documentId, name: current.name, pdfBase64: btoa(binary) }); setPdfMessage("PDF verdadeiro salvo separadamente no Google Drive."); if (result.webViewLink) window.open(result.webViewLink, "_blank", "noopener,noreferrer"); setPdfExport(null); }
