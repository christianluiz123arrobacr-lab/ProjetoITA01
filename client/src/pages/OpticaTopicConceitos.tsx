...
function getModuleKey(section: ContentSection, fallbackIndex: number) {
  const match = section.rawTitle.match(/^(\d+)/);
  return match ? match[1] : `misc-${fallbackIndex}`;
}

function buildTheoryModules(sections: ContentSection[]) {
  const modules: TheoryModule[] = [];
  const moduleMap = new Map<string, TheoryModule>();

  sections.forEach((section, index) => {
    const key = getModuleKey(section, index);
    ...
    const module = {
      key,
      title: stripLeadingNumber(section.title),
      sections: [section],
    };
      ...
      <table className="min-w-full divide-y divide-slate-200 text-sm">
        <thead className="bg-slate-950">
          <tr>
            {headers.map((header, index) => (
              <th key={index} className="px-4 py-3 text-left font-black text-white">
                <DarkInlineMathText text={header} />
              </th>
            ))}
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-200 bg-white">
          {rows.map((row, rowIndex) => (
            <tr key={rowIndex} className="hover:bg-indigo-50/60">
...
function NodeRenderer({ node }: { node: ContentNode }) {
  ...
  if (node.type === "subheading") {
    return (
      <h3 className="pt-3 text-lg font-black tracking-tight text-slate-950 md:text-xl">
        <InlineMathText text={node.text} />
      </h3>
    );
  }
  ...
  return <ContentTable headers={node.headers} rows={node.rows} />;
}

function ConnectedContentFrame({ children }: { children: ReactNode }) {
  return (
...
function TheoryModuleBlock({ module, index }: { module: TheoryModule; index: number }) {
  const Icon = sectionIcons[index % sectionIcons.length];
  const accent = sectionAccents[index % sectionAccents.length];

  return (
    <SectionCard icon={Icon} title={module.title} accent={accent}>
      <ConnectedContentFrame>
        {module.sections.map((section, sectionIndex) => (
          <div key={`${section.rawTitle}-${sectionIndex}`} className="space-y-5">
            {sectionIndex > 0 ? (
              <h3 className="pt-2 text-lg font-black tracking-tight text-slate-950 md:text-xl">
                <InlineMathText text={stripLeadingNumber(section.title)} />
              </h3>
            ) : null}

            {section.nodes.length > 0 ? (
              section.nodes.map((node, nodeIndex) => (
                <NodeRenderer key={`${section.rawTitle}-${nodeIndex}`} node={node} />
              ))
            ) : (
              <p>Este bloco organiza os conceitos seguintes da explicação.</p>
            )}
          </div>
        ))}
      </ConnectedContentFrame>
...
function ExampleAccordion({ section }: { section: ContentSection }) {
          ...
          <div>
            ...
            {firstParagraph?.type === "paragraph" ? (
              <p className="mt-2 text-sm leading-7 text-slate-600">
                <InlineMathText text={firstParagraph.text} />
              </p>
            ) : null}
          </div>
      ...
      {open && (
        <div className="space-y-5 border-t border-slate-100 p-6 leading-8 text-slate-700 md:p-7">
          {section.nodes.map((node, index) => (
            <NodeRenderer key={`${section.title}-${index}`} node={node} />
          ))}
        </div>
      )}
    </article>
  ...
  return (
    <div className="my-8 overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-50 via-white to-indigo-50 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
      <div className="border-b border-slate-200 bg-slate-950 px-6 py-4">
        <h3 className="text-lg font-black text-white">Mapa visual da Óptica Geométrica</h3>
        <p className="mt-1 text-sm leading-6 text-slate-300">
          Raio luminoso, normal, reflexão, refração e convergência em um mesmo painel.
        </p>
      </div>

      <div className="overflow-x-auto p-5 md:p-7">
        <svg viewBox="0 0 900 360" className="min-w-[760px] rounded-2xl bg-white">
          <defs>
            <marker id="arrow-optics" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth">
          ...
          <rect x="38" y="40" width="824" height="280" rx="28" fill="#f8fafc" stroke="#cbd5e1" strokeWidth="3" />
          <line x1="450" y1="70" x2="450" y2="290" stroke="#64748b" strokeDasharray="10 10" strokeWidth="4" />
          <line x1="85" y1="180" x2="815" y2="180" stroke="#1e293b" strokeWidth="5" />
          <text x="462" y="88" className="fill-slate-600 text-[18px] font-black">normal</text>
          <text x="110" y="165" className="fill-slate-700 text-[18px] font-black">interface</text>

          <line x1="170" y1="82" x2="448" y2="178" stroke="#0f172a" strokeWidth="5" markerEnd="url(#arrow-optics)" />
          <line x1="450" y1="180" x2="720" y2="86" stroke="#0f172a" strokeWidth="5" markerEnd="url(#arrow-optics)" />
          <line x1="450" y1="180" x2="640" y2="276" stroke="#2563eb" strokeWidth="5" markerEnd="url(#arrow-blue)" />

          <path d="M450 126 A54 54 0 0 0 397 162" fill="none" stroke="#f59e0b" strokeWidth="5" />
          <path d="M451 126 A54 54 0 0 1 502 162" fill="none" stroke="#f59e0b" strokeWidth="5" />
          <path d="M450 226 A52 52 0 0 0 497 204" fill="none" stroke="#2563eb" strokeWidth="5" />
          <text x="382" y="128" className="fill-amber-700 text-[20px] font-black">i</text>
          <text x="514" y="128" className="fill-amber-700 text-[20px] font-black">r</text>
          <text x="506" y="236" className="fill-blue-700 text-[20px] font-black">refração</text>

          <ellipse cx="185" cy="270" rx="70" ry="18" fill="#dbeafe" stroke="#2563eb" strokeWidth="3" />
          <line x1="115" y1="270" x2="255" y2="270" stroke="#2563eb" strokeWidth="4" markerEnd="url(#arrow-blue)" />
          <text x="110" y="312" className="fill-blue-800 text-[18px] font-black">feixe paralelo</text>

          <path d="M725 238 C772 208, 772 152, 725 122" fill="none" stroke="#7c3aed" strokeWidth="8" />
          <path d="M785 238 C738 208, 738 152, 785 122" fill="none" stroke="#7c3aed" strokeWidth="8" />
          <line x1="650" y1="150" x2="754" y2="172" stroke="#0f172a" strokeWidth="4" markerEnd="url(#arrow-optics)" />
          <line x1="650" y1="210" x2="754" y2="188" stroke="#0f172a" strokeWidth="4" markerEnd="url(#arrow-optics)" />
          <circle cx="812" cy="180" r="6" fill="#ef4444" />
          <text x="796" y="212" className="fill-red-700 text-[18px] font-black">F</text>
          <text x="700" y="92" className="fill-purple-800 text-[18px] font-black">lente convergente</text>
        </svg>
      </div>
    </div>
  );
...
export default function OpticaTopicConceitos() {
  ...
  const groupedSections = useMemo(() => {
    ...
    const examples = contentSections.filter(
      (section) => section.group === "examples" && section.nodes.length > 0,
    );
    const summary = contentSections.filter((section) => section.group === "summary");
    const theoryModules = buildTheoryModules(theory);

    return { theoryModules, examples, summary };
  }, []);
      ...
      <main className="mx-auto max-w-6xl space-y-10 px-4 py-8 md:px-6 md:py-10">
        {activeTab === "teoria" && (
          <>
              ...
              <div className="relative grid items-end gap-8 lg:grid-cols-[1.35fr_0.65fr]">
                <div>
                  ...
                  <h2 className="max-w-4xl text-4xl font-black leading-tight tracking-tight text-white md:text-6xl">
                    Luz, imagens, espelhos, refração, lentes e instrumentos.
                  </h2>

                  <p className="mt-5 max-w-3xl leading-8 text-slate-300">
                    Uma página completa de Óptica no mesmo padrão visual das aulas fortes do site: explicação conectada, fórmulas interpretadas, blocos de prova e exemplos sem transformar a matéria numa feira de cards aleatórios.
                  </p>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  {[
                    [String(groupedSections.theoryModules.length), "módulos"],
                    [String(groupedSections.examples.length), "exemplos"],
                    [String(formulaSummary.length), "fórmulas"],
                    ["ITA", "nível"],
                  ].map(([value, label]) => (
                    <div
                      key={label}
            ...
            <SectionCard icon={Compass} title="Como a página foi organizada" accent="from-slate-950 to-indigo-900">
              <div className="grid gap-5 md:grid-cols-2">
                <MiniInfoCard title="Sequência de estudo">
                  <p>
                    Primeiro vêm natureza da luz, princípios e meios ópticos. Depois entram reflexão, espelhos, refração, lentes, instrumentos e visão. É a ordem mais limpa para entender antes de calcular.
                  </p>
                </MiniInfoCard>

                <MiniInfoCard title="Leitura de prova">
                  <p>
                    Em Óptica, desenho, normal, sinal e posição da imagem importam tanto quanto fórmula. Decorar sem desenhar é basicamente pedir para a prova te atropelar de ré.
                  </p>
                </MiniInfoCard>
              </div>
            ...
            {groupedSections.theoryModules.map((module, index) => (
              <TheoryModuleBlock key={module.key} module={module} index={index} />
            ))}
          </>
        )}
        ...
        {activeTab === "exemplos" && (
          <>
            <CompactTabHeader
              ...
              title="Exemplos resolvidos de Óptica"
              description="Câmara escura, espelhos planos, espelhos esféricos, Snell, reflexão total, profundidade aparente, lentes e instrumentos com interpretação antes da conta. Quase civilizado."
              accent="from-slate-950 via-indigo-950 to-purple-950"
            />

            <div className="space-y-5">
              {groupedSections.examples.map((section) => (
                <ExampleAccordion key={section.rawTitle} section={section} />
              ))}
            </div>
          </>
        )}
        ...
        {activeTab === "resumo" && (
          <>
            <CompactTabHeader
              icon={Brain}
              eyebrow="Mapa final"
              title="Resumo de Óptica"
              description="Fórmulas essenciais, diferenças conceituais e armadilhas que mais aparecem em vestibulares fortes."
            ...
            >
              <BulletList
                items={[
                  "Raio luminoso é modelo geométrico; onda eletromagnética é descrição física mais completa.",
                  "Na refração, a frequência permanece constante; velocidade e comprimento de onda mudam.",
                  "Ângulos de incidência, reflexão e refração são sempre medidos em relação à normal.",
                  "Imagem real nasce do cruzamento efetivo dos raios e pode ser projetada em tela.",
                  "Imagem virtual nasce do cruzamento dos prolongamentos e não pode ser projetada diretamente em tela.",
                  "Espelho convexo sempre forma imagem virtual, direita e menor para objeto real.",
                  "Lente divergente sempre forma imagem virtual, direita e menor para objeto real.",
                  "Reflexão total só ocorre do meio mais refringente para o menos refringente e com ângulo maior que o limite.",
                  "Vergência depende de f em metros; esquecer isso é um clássico desastre humano com unidades.",
                ]}
              />

              <NoteBox title="Observações" type="dark">
                Óptica não é só substituir número em Gauss e Snell. A prova quer saber se você entende o caminho da luz, a posição da imagem e a convenção usada. Fórmula sem figura aqui é quase superstição com trigonometria.
              </NoteBox>
            </SectionCard>

            {groupedSections.summary.length > 0 ? (
              <SectionCard icon={BookOpen} title="Resumo textual completo" accent="from-blue-600 to-cyan-600">
                <ConnectedContentFrame>
                  {groupedSections.summary.map((section, sectionIndex) => (
                    <div key={`${section.rawTitle}-${sectionIndex}`} className="space-y-5">
                      <h3 className="pt-2 text-lg font-black tracking-tight text-slate-950 md:text-xl">
                        <InlineMathText text={stripLeadingNumber(section.title)} />
                      </h3>
