...
export default function NotebookPage() {
  ...
  if (status.isLoading)
    return (
      <main className="theme-page min-h-screen bg-slate-50 p-8 dark:bg-slate-950">
        <p className="text-slate-600 dark:text-slate-300">Verificando Google Drive...</p>
      </main>
    );
  if (
    status.isError ||
    ...
    return (
      <main className="theme-page min-h-screen bg-slate-50 p-6 dark:bg-slate-950">
        <section className="mx-auto max-w-2xl rounded-3xl border border-rose-200 bg-white p-8 text-center shadow-sm dark:border-rose-900 dark:bg-slate-900">
  ...
  return (
    <main className="theme-page min-h-screen bg-slate-50 px-4 py-8 dark:bg-slate-950 sm:px-6 lg:px-10">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="flex flex-col gap-4 rounded-3xl border border-blue-100 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900 sm:flex-row sm:items-center sm:justify-between">
        ...
        {status.data?.requiresReconnect ? (
          <section className="rounded-3xl border border-amber-200 bg-white p-8 text-center shadow-sm dark:border-amber-900 dark:bg-slate-900">
          ...
          <section className="rounded-3xl border border-rose-200 bg-white p-8 text-center dark:border-rose-900 dark:bg-slate-900">
          ...
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {notebooks.data?.map(item => (
              ...
              >
                ...
                <h2 className="mt-4 truncate text-lg font-black text-slate-900 dark:text-slate-100">
                  {item.name}
                </h2>
                ...
                <p className="mt-1 text-xs font-bold uppercase text-cyan-700 dark:text-cyan-300">
                  {item.paper.size} •{" "}
                ...
                <div className="mt-5 flex gap-2">
                  ...
                  <Button
                    variant="outline"
                    className="text-rose-600 dark:border-slate-600 dark:text-rose-300"
                    onClick={() =>
            ...
            <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-xl dark:border-slate-700 dark:bg-slate-900">
              ...
              <label className="mt-4 block text-sm font-bold">
                ...
                <input
                  ...
                  className="mt-1 h-11 w-full rounded-xl border px-3 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
                />
              </label>
              ...
              <fieldset className="mt-4">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  {(["a5", "a4", "a3", "infinite"] as const).map(size => (
                    <label key={size} className="rounded-xl border p-3 dark:border-slate-600 dark:bg-slate-800">
                      <input
                        type="radio"
                        checked={paper === size}
