...
import { isMultitouchGesture, shouldNavigateInsteadOfDraw } from "./studyCanvasPointerGuards";
...
export type StudyCanvasPage = {
  ...
  /** Immutable page background, used by imported documents. */
...
type StudyCanvasWorkspaceProps = {
  ...
  onPdfReady?: (pdf: Blob, fileName: string) => void;
  compactMode?: boolean;
  drawWithTouchInitially?: boolean;
  paperSize?: "a5" | "a4" | "a3" | "infinite";
  /** Fingers only navigate; stylus and mouse can still draw. */
  lockTouchToPan?: boolean;
};
...
type CompactPanel = "pages" | "tools" | "style" | "view" | null;
    ...
    .map((page, index) => {
      ...
      return {
        ...
        backgroundType: isValidBackground(maybePage.backgroundType)
          ...
          : fallbackBackground,
        backgroundImageData: typeof maybePage.backgroundImageData === "string"
          ? maybePage.backgroundImageData
          : undefined,
      };
    })
    .filter((page) => page.id);
...
function clampView(view: CanvasView, canvasWidth = CANVAS_WIDTH, canvasHeight = CANVAS_HEIGHT): CanvasView {
  ...
  const minX = canvasWidth - canvasWidth * zoom - PAN_MARGIN;
  const maxX = PAN_MARGIN;
  const minY = canvasHeight - canvasHeight * zoom - PAN_MARGIN;
...
function getVisibleLogicalRect(view: CanvasView, canvasWidth = CANVAS_WIDTH, canvasHeight = CANVAS_HEIGHT) {
  return {
    left: (-view.offsetX - 30) / view.zoom,
    right: (canvasWidth - view.offsetX + 30) / view.zoom,
    top: (-view.offsetY - 30) / view.zoom,
    bottom: (canvasHeight - view.offsetY + 30) / view.zoom,
  };
}
...
function drawBackground(
  ...
  const visible = getVisibleLogicalRect(view, canvasWidth, canvasHeight);
...
function drawPaper(
  ...
  drawBackground(ctx, view, backgroundType, canvasWidth, canvasHeight);
  ctx.restore();
}
...
function drawPenStroke(ctx: CanvasRenderingContext2D, stroke: ScratchpadStroke) {
  ...
  ctx.save();
  const profile = BRUSH_RENDERING[stroke.brush ?? "pen"];
...
export function StudyCanvasWorkspace({
  ...
  onPdfReady,
  compactMode = false,
  drawWithTouchInitially = true,
  paperSize = "a4",
  lockTouchToPan = false,
  ...
  const [drawWithTouch, setDrawWithTouch] = useState(drawWithTouchInitially);
  const [compactPanel, setCompactPanel] = useState<CompactPanel>(null);

  useEffect(() => {
    if (lockTouchToPan) setDrawWithTouch(false);
  }, [lockTouchToPan]);
  ...
  const canvasDimensions = useMemo(() => {
    if (!compactMode) return { width: CANVAS_WIDTH, height: CANVAS_HEIGHT };
    return getPaperDimensions(paperSize);
  }, [compactMode, paperSize]);
  ...
  function renameCurrentPage() {
    ...
    if (!current) return;
    const title = window.prompt("Nome da página:", current.title)?.trim();
    if (!title) return;
    setPages(previous => previous.map(page => page.id === activePageId ? { ...page, title: title.slice(0, 80) } : page));
    markDirty("Página renomeada.");
  }

  function moveCurrentPage(direction: -1 | 1) {
    const persisted = getPagesForPersistence();
    const index = persisted.findIndex(page => page.id === activePageId);
    const target = index + direction;
    if (index < 0 || target < 0 || target >= persisted.length) return;
    const next = [...persisted];
    [next[index], next[target]] = [next[target], next[index]];
    setPages(next);
    markDirty("Ordem das páginas alterada.");
  }
  ...
  const updateView = useCallback((nextView: CanvasView | ((previous: CanvasView) => CanvasView)) => {
    setView((previous) => {
      ...
      const clamped = clampView(rawNext, canvasDimensions.width, canvasDimensions.height);
  ...
  const getRawCanvasPoint = useCallback((clientX: number, clientY: number) => {
    ...
    return {
      x: ((clientX - rect.left) / rect.width) * canvasDimensions.width,
      y: ((clientY - rect.top) / rect.height) * canvasDimensions.height,
    };
  }, [canvasDimensions.height, canvasDimensions.width]);
  ...
  function getCommittedInkLayer(pixelRatio: number) {
    const width = Math.round(canvasDimensions.width * pixelRatio);
    const height = Math.round(canvasDimensions.height * pixelRatio);
  ...
  const redrawCanvas = useCallback(
    (
      ...
      const pixelRatio = ensureCanvasBitmap(canvas, canvasDimensions.width, canvasDimensions.height);
      ...
      drawPaper(ctx, viewRef.current, backgroundTypeRef.current, pixelRatio, canvasDimensions.width, canvasDimensions.height);
    ...
    [canvasDimensions.height, canvasDimensions.width, selectedIds]
  );
  ...
  useEffect(() => {
    ...
    ensureCanvasBitmap(canvas, canvasDimensions.width, canvasDimensions.height);

    redrawCanvas();
  }, [canvasDimensions.height, canvasDimensions.width, open, fullscreen, redrawCanvas]);
  ...
  const handleSave = useCallback(
    ...
    [activePageId, canSave, canvasDimensions.height, canvasDimensions.width, disableLocalDraft, pages, persistence, questionId, title, userId]
  );
  ...
  }, [activePageId, canSave, canvasDimensions.height, canvasDimensions.width, pages, persistence, questionId, title, userId]);
  ...
  function handlePointerDown(event: React.PointerEvent<HTMLCanvasElement>) {
    ...
    const touchMustNavigate = shouldNavigateInsteadOfDraw(event.pointerType, lockTouchToPan, drawWithTouch);
    if (touchMustNavigate) {
    ...
    if (tool === "pen") {
      ...
      const stroke: ScratchpadStroke = {
        ...
        size,
        brush: activeBrush,
        opacity: BRUSH_RENDERING[activeBrush].opacity,
  ...
  function toggleCompactPanel(panel: Exclude<CompactPanel, null>) {
  ...
  function copySelection() {
    const selectedSet = new Set(selectedIds);
    clipboardRef.current = cloneStrokes(strokesRef.current.filter(stroke => selectedSet.has(stroke.id)));
    setStatus(`${clipboardRef.current.length} item(ns) copiado(s).`);
  }

  function pasteSelection() {
    if (!clipboardRef.current.length) return;
    const copies = clipboardRef.current.map(stroke => ({ ...cloneStrokes([stroke])[0], id: createStrokeId(), points: stroke.points.map(point => ({ ...point, x: point.x + 32, y: point.y + 32 })) }));
    const previous = cloneStrokes(strokesRef.current);
    setSelectedIds(copies.map(stroke => stroke.id));
    commitStrokes([...strokesRef.current, ...copies], previous);
  }

  function setSelectionColor(nextColor: string) {
    applyTransformToSelection(stroke => ({ ...stroke, color: nextColor }));
  }

  function reorderSelection(position: "front" | "back") {
    if (!selectedIds.length) return;
    const selectedSet = new Set(selectedIds);
    const previous = cloneStrokes(strokesRef.current);
    const selected = strokesRef.current.filter(stroke => selectedSet.has(stroke.id));
    const rest = strokesRef.current.filter(stroke => !selectedSet.has(stroke.id));
    commitStrokes(position === "front" ? [...rest, ...selected] : [...selected, ...rest], previous);
  }

  function alignSelection(axis: "horizontal" | "vertical") {
    if (selectedIds.length < 2) return;
    const bounds = getSelectedBounds(strokesRef.current, selectedIds);
    if (!bounds) return;
    const target = axis === "horizontal" ? (bounds.minY + bounds.maxY) / 2 : (bounds.minX + bounds.maxX) / 2;
    applyTransformToSelection(stroke => {
      const strokeBounds = getStrokeBounds(stroke);
      if (!strokeBounds) return stroke;
      const current = axis === "horizontal" ? (strokeBounds.minY + strokeBounds.maxY) / 2 : (strokeBounds.minX + strokeBounds.maxX) / 2;
      return transformStrokePoints(stroke, point => axis === "horizontal" ? { ...point, y: point.y + target - current } : { ...point, x: point.x + target - current }, 1);
    });
  }

  function renderPageToDataUrl(page: ScratchpadPage, pixelRatio = 2, format: "image/png" | "image/jpeg" = "image/png") {
    const exportCanvas = document.createElement("canvas");
    exportCanvas.width = canvasDimensions.width * pixelRatio;
    exportCanvas.height = canvasDimensions.height * pixelRatio;

    const ctx = exportCanvas.getContext("2d");
    if (!ctx) return "";

    const exportView: CanvasView = { zoom: 1, offsetX: 0, offsetY: 0 };
    drawPaper(ctx, exportView, page.backgroundType, pixelRatio, canvasDimensions.width, canvasDimensions.height);
    ...
    if (format === "image/jpeg") {
      ...
      ctx.fillRect(0, 0, canvasDimensions.width, 38);
      ctx.fillStyle = "#334155";
      ctx.font = "bold 13px Arial, sans-serif";
      ctx.fillText(`Projeto Vetor · ${title} · ${new Date().toLocaleDateString("pt-BR")}`, 18, 24);
      ctx.restore();
    }

    return exportCanvas.toDataURL(format, format === "image/jpeg" ? 0.92 : undefined);
  }
  ...
  function exportAsPdf() {
    ...
    let printSize: PrintPaperSize = paperSize === "infinite" ? "a4" : paperSize;
    if (paperSize === "infinite") {
      const choice = window.prompt("Formato de impressão do papel infinito: A4, A3 ou A5", "A4")?.trim().toLowerCase();
      if (!choice) return;
      if (!(["a4", "a3", "a5"] as const).includes(choice as PrintPaperSize)) { window.alert("Escolha A4, A3 ou A5."); return; }
      printSize = choice as PrintPaperSize;
    }
    const pdf = buildVectorStudyCanvasPdf({
      pages: pagesForExport.map(page => ({ elements: page.strokes })),
      title,
      sourceWidth: canvasDimensions.width,
      sourceHeight: canvasDimensions.height,
      paperSize: printSize,
      lined: backgroundTypeRef.current === "lined",
      paginateInfinite: paperSize === "infinite",
    });
    const fileName = safePdfFileName(title);
    if (onPdfReady) { onPdfReady(pdf, fileName); return; }
    const url = URL.createObjectURL(pdf); const link = document.createElement("a"); link.href = url; link.download = fileName; link.click(); window.setTimeout(() => URL.revokeObjectURL(url), 1_000);
  }
  ...
  function importImageFile(file: File) {
    if (file.type === "application/pdf") {
      setError("A importação de PDF está indisponível neste ambiente. Envie PNG, JPG, WEBP ou SVG; a exportação continua gerando PDF binário real.");
      return;
    }
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if ((event.ctrlKey || event.metaKey) && key === "c" && selectedIds.length > 0) {
        event.preventDefault(); copySelection(); return;
      }
      if ((event.ctrlKey || event.metaKey) && key === "v" && clipboardRef.current.length > 0) {
  ...
  const containerClassName = fullscreen
    ...
    : compactMode
      ? "h-full overflow-hidden bg-slate-100"
  ...
  const canvasWrapperClassName = fullscreen
    ? "flex min-h-0 flex-1 flex-col overflow-hidden rounded-none border-0 bg-black p-0"
    : compactMode
      ? "flex min-h-[calc(100dvh-150px)] flex-col overflow-hidden rounded-xl bg-slate-200/70 shadow-inner"
      : "rounded-2xl border border-slate-200 bg-white p-2 shadow-inner";

  const paperVisualStyle = compactMode
    ? getPaperViewportStyle(paperSize, fullscreen, view.zoom)
    : undefined;
    ...
    <div ref={fullscreenRootRef} className={containerClassName}>
      ...
      {open ? (
        <div className={`${fullscreen ? "flex min-h-0 flex-1 flex-col gap-0 overflow-hidden bg-black p-0" : compactMode ? "h-full p-0" : "border-t border-violet-100 p-4 md:p-5"}`}>
          ...
          <div className={`${compactMode && !fullscreen ? "hidden" : ""} ${fullscreen ? "mb-0 flex-nowrap overflow-x-auto border-b border-slate-800 bg-slate-950 px-3 py-2 text-slate-100" : "mb-4 flex-wrap rounded-3xl border border-slate-200 bg-white p-3 shadow-sm"} flex shrink-0 items-center gap-2`}>
                ...
                <div className="flex min-h-[44px] items-center gap-2 overflow-x-auto border-t border-slate-700 bg-[#2b2b2b] px-3 py-2">
                  ...
                  {fullscreenPanel === "selection" ? (
                    <>
                      ...
                      <button type="button" onClick={copySelection} className="shrink-0 rounded-full bg-slate-800 px-3 py-2 text-xs font-black text-slate-100">Copiar</button>
                      <button type="button" onClick={pasteSelection} className="shrink-0 rounded-full bg-slate-800 px-3 py-2 text-xs font-black text-slate-100">Colar</button>
                      <button type="button" onClick={() => reorderSelection("front")} className="shrink-0 rounded-full bg-slate-800 px-3 py-2 text-xs font-black text-slate-100">Frente</button>
                      <button type="button" onClick={() => reorderSelection("back")} className="shrink-0 rounded-full bg-slate-800 px-3 py-2 text-xs font-black text-slate-100">Fundo</button>
                  ...
                  {fullscreenPanel === "view" ? (
                    <>
                      <label className="flex shrink-0 items-center gap-2 rounded-full bg-slate-800 px-3 py-2 text-xs font-black text-slate-100"><input type="checkbox" checked={drawWithTouch} onChange={event => setDrawWithTouch(event.target.checked)} />Desenhar com dedo</label>
          ...
          <div className={fullscreen || compactMode ? "hidden" : "mb-4 flex shrink-0 flex-wrap items-center gap-3 rounded-3xl border border-slate-200 bg-white p-3 shadow-sm"}>
            ...
            <ToolbarGroup label="Ajustes">
              <label className="flex items-center gap-2 text-sm font-bold text-slate-700"><input type="checkbox" checked={drawWithTouch} onChange={event => setDrawWithTouch(event.target.checked)} />Desenhar com dedo</label>
          ...
          {selectedIds.length > 0 && !fullscreen ? (
            <div className="mb-4 flex shrink-0 flex-wrap items-center gap-2 rounded-2xl border border-violet-200 bg-violet-50 p-3 text-sm">
              ...
              <button type="button" onClick={copySelection} className="rounded-xl bg-white px-3 py-2 font-bold text-violet-700 ring-1 ring-violet-200">Copiar</button>
              <button type="button" onClick={pasteSelection} className="rounded-xl bg-white px-3 py-2 font-bold text-violet-700 ring-1 ring-violet-200">Colar</button>
              <button type="button" onClick={() => reorderSelection("front")} className="rounded-xl bg-white px-3 py-2 font-bold text-violet-700 ring-1 ring-violet-200">Trazer à frente</button>
              <button type="button" onClick={() => reorderSelection("back")} className="rounded-xl bg-white px-3 py-2 font-bold text-violet-700 ring-1 ring-violet-200">Enviar para trás</button>
              {selectedIds.length > 1 ? <><button type="button" onClick={() => alignSelection("horizontal")} className="rounded-xl bg-white px-3 py-2 font-bold text-violet-700 ring-1 ring-violet-200">Centralizar horizontal</button><button type="button" onClick={() => alignSelection("vertical")} className="rounded-xl bg-white px-3 py-2 font-bold text-violet-700 ring-1 ring-violet-200">Centralizar vertical</button></> : null}
              <div className="flex gap-1">{COLORS.map(item => <button key={item} type="button" aria-label={`Aplicar cor ${item} à seleção`} onClick={() => setSelectionColor(item)} className="h-7 w-7 rounded-full ring-1 ring-slate-300" style={{ backgroundColor: item }} />)}</div>
          ...
          <div className={canvasWrapperClassName}>
            {compactMode && !fullscreen ? (
              <div className="relative z-30 mx-auto w-full max-w-5xl shrink-0 p-2 pb-0">
                <div className="flex flex-wrap items-center gap-1.5 rounded-2xl border border-slate-700 bg-slate-950/95 p-1.5 text-white shadow-xl backdrop-blur">
                  {(["pages", "tools", "style", "view"] as const).map(panel => (
                    <button key={panel} type="button" onClick={() => toggleCompactPanel(panel)} className={`rounded-xl px-3 py-2 text-xs font-black transition ${compactPanel === panel ? "bg-cyan-400 text-slate-950" : "bg-slate-800 text-slate-100 hover:bg-slate-700"}`}>
                      {panel === "pages" ? `Páginas (${pages.length})` : panel === "tools" ? getCurrentToolLabel() : panel === "style" ? "Estilo" : "Visual"}
                    </button>
                  ))}
                  <div className="mx-1 hidden h-6 w-px bg-slate-700 sm:block" />
                  <button type="button" onClick={handleUndo} disabled={!historyDepth} aria-label="Desfazer" title="Desfazer" className="rounded-xl p-2 hover:bg-slate-800 disabled:opacity-35"><Undo2 className="h-4 w-4" /></button>
                  <button type="button" onClick={handleRedo} disabled={!redoDepth} aria-label="Refazer" title="Refazer" className="rounded-xl p-2 hover:bg-slate-800 disabled:opacity-35"><Redo2 className="h-4 w-4" /></button>
                  <button type="button" onClick={() => handleSave()} disabled={!canSave || saving} aria-label="Salvar agora" title="Salvar agora" className="ml-auto rounded-xl bg-violet-600 p-2 disabled:opacity-40">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}</button>
                  <button type="button" onClick={exportAsPdf} aria-label="Exportar PDF" title="Exportar PDF" className="flex items-center gap-1 rounded-xl bg-cyan-400 px-3 py-2 text-xs font-bold text-slate-950"><Download className="h-4 w-4" /><span>Exportar PDF</span></button>
                  <button type="button" onClick={toggleScratchpadFullscreen} aria-label="Tela cheia" title="Tela cheia" className="rounded-xl bg-slate-100 p-2 text-slate-950"><Maximize2 className="h-4 w-4" /></button>
                </div>
                {compactPanel ? <div className="mt-2 rounded-2xl border border-slate-700 bg-slate-900/95 p-2.5 text-white shadow-xl backdrop-blur">
                  {compactPanel === "pages" ? <div className="flex flex-wrap items-center gap-2">
                    {pages.map((page, index) => {
                      const thumbnailPage = page.id === activePageId
                        ? { ...page, strokes: strokesRef.current, backgroundType: backgroundTypeRef.current }
                        : page;
                      return (
                        <button
                          key={page.id}
                          type="button"
                          onClick={() => { openPage(page.id); setCompactPanel(null); }}
                          className={`w-24 overflow-hidden rounded-xl p-1 text-xs font-black ${page.id === activePageId ? "bg-cyan-400 text-slate-950" : "bg-slate-800"}`}
                          title={page.title}
                        >
                          <img
                            src={renderPageToDataUrl(thumbnailPage, 0.12, "image/jpeg")}
                            alt={`Miniatura da ${page.title}`}
                            className="mb-1 aspect-[1/1.414] w-full rounded-md bg-white object-cover object-top"
                          />
                          <span className="block truncate">{index + 1}. {page.title}</span>
                        </button>
                      );
                    })}
                    <button type="button" onClick={addPage} className="h-10 rounded-xl bg-cyan-400 px-3 text-xs font-black text-slate-950">+ Nova página</button>
                    <button type="button" onClick={renameCurrentPage} className="h-10 rounded-xl bg-slate-800 px-3 text-xs font-black">Renomear</button>
                    <button type="button" onClick={duplicateCurrentPage} className="h-10 rounded-xl bg-slate-800 px-3 text-xs font-black">Duplicar</button>
                    <button type="button" onClick={() => moveCurrentPage(-1)} className="h-10 rounded-xl bg-slate-800 px-3 text-xs font-black">Mover ↑</button>
                    <button type="button" onClick={() => moveCurrentPage(1)} className="h-10 rounded-xl bg-slate-800 px-3 text-xs font-black">Mover ↓</button>
                    <button type="button" onClick={deleteCurrentPage} className="h-10 rounded-xl bg-red-950 px-3 text-xs font-black text-red-200">Excluir</button>
                  </div> : null}
                  {compactPanel === "tools" ? <div className="flex flex-wrap gap-2">
                    <button type="button" onClick={() => { activatePen("pen"); setCompactPanel(null); }} className={toolButtonClass(tool === "pen" && brush === "pen")}><PenLine className="h-4 w-4" /> Caneta</button>
                    <button type="button" onClick={() => { activatePen("brush"); setCompactPanel(null); }} className={toolButtonClass(tool === "pen" && brush === "brush")}><PenLine className="h-4 w-4" /> Pincel</button>
                    <button type="button" onClick={() => { activatePen("highlighter"); setCompactPanel(null); }} className={toolButtonClass(tool === "pen" && brush === "highlighter")}><Highlighter className="h-4 w-4" /> Marca-texto</button>
                    <button type="button" onClick={() => { setTool("areaEraser"); setCompactPanel(null); }} className={toolButtonClass(tool === "areaEraser")}><Eraser className="h-4 w-4" /> Borracha</button>
                    <button type="button" onClick={() => { setTool("strokeEraser"); setCompactPanel(null); }} className={toolButtonClass(tool === "strokeEraser")}><Eraser className="h-4 w-4" /> Traço inteiro</button>
                  ...
                  {compactPanel === "style" ? <div className="flex flex-wrap items-center gap-3">
                    ...
                    <label className="flex items-center gap-2 text-xs font-black">Espessura <input type="range" min={2} max={18} value={size} onChange={event => setSize(Number(event.target.value))} className="w-28 accent-cyan-400" />{size}</label>
                    {(tool === "areaEraser" || tool === "strokeEraser") ? <label className="flex items-center gap-2 text-xs font-black">Borracha <input type="range" min={10} max={80} value={eraserSize} onChange={event => setEraserSize(Number(event.target.value))} className="w-28 accent-cyan-400" />{eraserSize}</label> : null}
                  </div> : null}
                  {compactPanel === "view" ? <div className="flex flex-wrap items-center gap-2">
                    <select value={backgroundType} onChange={event => handleBackgroundChange(event.target.value as ScratchpadBackground)} className="rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-xs font-black text-white">{BACKGROUNDS.map(item => <option key={item.value} value={item.value}>Fundo: {item.label}</option>)}</select>
                    <button type="button" onClick={() => handleZoomButton("out")} className="rounded-xl bg-slate-800 px-3 py-2 text-xs font-black">Zoom −</button>
                    <button type="button" onClick={resetZoom} className="rounded-xl bg-slate-800 px-3 py-2 text-xs font-black">{Math.round(view.zoom * 100)}%</button>
                    <button type="button" onClick={() => handleZoomButton("in")} className="rounded-xl bg-slate-800 px-3 py-2 text-xs font-black">Zoom +</button>
                    {!lockTouchToPan ? <label className="flex items-center gap-2 rounded-xl bg-slate-800 px-3 py-2 text-xs font-black"><input type="checkbox" checked={drawWithTouch} onChange={event => setDrawWithTouch(event.target.checked)} />Desenhar com dedo</label> : <span className="rounded-xl bg-emerald-950 px-3 py-2 text-xs font-black text-emerald-200">Toque: mover e zoom</span>}
                  </div> : null}
                </div> : null}
              </div>
            ) : null}
            {!loaded ? (
            ...
            ) : compactMode && paperVisualStyle ? (
              <PaperViewport
                fullscreen={fullscreen}
                frameStyle={paperVisualStyle}
              ...
              >
                <canvas
                  ...
                  className={`block h-full w-full bg-white ${canvasCursorClass}`}
              ...
              <canvas
                ...
                onWheel={handleWheel}
                onDoubleClick={handleCanvasDoubleClick}
                className={`block w-full rounded-xl bg-white ${canvasCursorClass}`}
                style={{ aspectRatio: `${canvasDimensions.width} / ${canvasDimensions.height}`, touchAction: "none" }}
              />
            )}
          </div>
