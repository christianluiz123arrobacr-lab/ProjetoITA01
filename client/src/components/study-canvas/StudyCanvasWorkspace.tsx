...
export function StudyCanvasWorkspace({
  ...
  function handlePointerDown(event: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (event.pointerType === "touch" && !drawWithTouch) {
      ...
      {open ? (
        <div className={`${fullscreen ? "flex min-h-0 flex-1 flex-col gap-0 overflow-hidden bg-black p-0" : compactMode ? "h-full p-0" : "border-t border-violet-100 p-4 md:p-5"}`}>
          ...
          <div className={canvasWrapperClassName}>
            {compactMode && !fullscreen ? <div className="absolute left-1/2 top-2 z-30 flex max-w-[60%] -translate-x-1/2 items-center gap-1 overflow-x-auto rounded-full bg-slate-900/95 px-2 py-1.5 text-white shadow-xl">
              <span className="shrink-0 px-2 text-xs font-bold">{getCurrentToolLabel()}</span>
              <input type="color" value={color} onChange={event => setColor(event.target.value)} aria-label="Cor atual" className="h-7 w-7 shrink-0 rounded-full border-0 bg-transparent" />
              <input aria-label="Espessura" type="range" min={2} max={18} value={size} onChange={event => setSize(Number(event.target.value))} className="w-20 accent-cyan-400" />
              <label className="flex shrink-0 items-center gap-1 px-1 text-[10px] font-bold"><input type="checkbox" checked={drawWithTouch} onChange={event => setDrawWithTouch(event.target.checked)} />Dedo</label>
              <button type="button" onClick={handleUndo} disabled={!historyDepth} aria-label="Desfazer" className="rounded-full p-2 disabled:opacity-40"><Undo2 className="h-4 w-4" /></button>
              <button type="button" onClick={handleRedo} disabled={!redoDepth} aria-label="Refazer" className="rounded-full p-2 disabled:opacity-40"><Redo2 className="h-4 w-4" /></button>
              <button type="button" onClick={() => handleSave()} disabled={!canSave || saving} aria-label="Salvar" className="rounded-full p-2"><Save className="h-4 w-4" /></button>
              <button type="button" onClick={exportAsPdf} aria-label="Exportar PDF" className="rounded-full p-2"><Download className="h-4 w-4" /></button>
            </div> : null}
            {compactMode && !fullscreen ? (
              <aside className="sticky left-0 top-2 z-20 mr-3 flex w-16 shrink-0 flex-col items-center gap-2 rounded-2xl bg-slate-900/95 p-2 text-white shadow-xl" aria-label="Páginas">
                {pages.map((page, index) => <button key={page.id} type="button" onClick={() => openPage(page.id)} title={page.title} className={`h-12 w-11 rounded-lg border text-xs font-black ${page.id === activePageId ? "border-cyan-300 bg-blue-600" : "border-slate-600 bg-white text-slate-700"}`}>{index + 1}</button>)}
                <button type="button" onClick={addPage} title="Nova página" className="h-9 w-9 rounded-full bg-cyan-500 text-lg font-black">+</button>
                <button type="button" onClick={renameCurrentPage} title="Renomear página" className="text-xs text-slate-200">Nome</button>
                <button type="button" onClick={duplicateCurrentPage} title="Duplicar página" className="text-xs text-slate-200">Duplicar</button>
                <div className="flex gap-1"><button type="button" onClick={() => moveCurrentPage(-1)} title="Mover página para cima">↑</button><button type="button" onClick={() => moveCurrentPage(1)} title="Mover página para baixo">↓</button></div>
                <button type="button" onClick={deleteCurrentPage} title="Excluir página" className="text-xs text-red-300">Excluir</button>
              </aside>
            ) : null}
            {!loaded ? (
            ...
            {compactMode && !fullscreen ? <div className="sticky right-2 top-2 z-20 ml-3 flex h-fit w-12 shrink-0 flex-col gap-1 rounded-2xl bg-slate-900/95 p-1.5 shadow-xl">
              <button type="button" aria-label="Caneta" title="Caneta" onClick={() => activatePen("pen")} className={toolButtonClass(tool === "pen" && brush === "pen")}><PenLine className="h-4 w-4" /></button>
              <button type="button" aria-label="Pincel" title="Pincel" onClick={() => activatePen("brush")} className={toolButtonClass(tool === "pen" && brush === "brush")}><Highlighter className="h-4 w-4" /></button>
              <button type="button" aria-label="Borracha" title="Borracha" onClick={() => setTool("areaEraser")} className={toolButtonClass(tool === "areaEraser")}><Eraser className="h-4 w-4" /></button>
              <button type="button" aria-label="Selecionar" title="Selecionar" onClick={() => setTool("select")} className={toolButtonClass(tool === "select")}><MousePointer2 className="h-4 w-4" /></button>
              <button type="button" aria-label="Texto" title="Texto" onClick={() => setTool("text")} className={toolButtonClass(tool === "text")}><Type className="h-4 w-4" /></button>
              <button type="button" aria-label="Forma" title="Forma" onClick={() => activateShape("line")} className={toolButtonClass(tool === "shape")}><Square className="h-4 w-4" /></button>
              <button type="button" aria-label="Mover" title="Mover" onClick={() => setTool("pan")} className={toolButtonClass(tool === "pan")}><Move className="h-4 w-4" /></button>
              <button type="button" aria-label="Tela cheia" title="Tela cheia" onClick={toggleScratchpadFullscreen} className={toolButtonClass(false)}><Maximize2 className="h-4 w-4" /></button>
            </div> : null}
            {compactMode && !fullscreen ? <div className="fixed bottom-4 left-1/2 z-30 flex -translate-x-1/2 items-center gap-2 rounded-full bg-slate-900 px-3 py-2 text-xs font-bold text-white shadow-xl"><button onClick={() => handleZoomButton("out")} aria-label="Diminuir zoom">−</button><span>{Math.round(view.zoom * 100)}%</span><button onClick={() => handleZoomButton("in")} aria-label="Aumentar zoom">+</button></div> : null}
          </div>
