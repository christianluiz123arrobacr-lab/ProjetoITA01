...
import { buildStudyCanvasPdf, safePdfFileName } from "@/lib/studyCanvasPdf";
...
export function StudyCanvasWorkspace({
  ...
  function exportAsPdf() {
    ...
    const jpegPages = pagesForExport.map(page => ({ dataUrl: renderPageToDataUrl(page, 2, "image/jpeg"), width: canvasDimensions.width * 2, height: canvasDimensions.height * 2 }));
    const pdf = buildStudyCanvasPdf(jpegPages, title);
            ...
            {compactMode && !fullscreen ? (
              <div className="relative z-30 mx-auto w-full max-w-5xl shrink-0 p-2 pb-0">
                <div className="flex flex-wrap items-center gap-1.5 rounded-2xl border border-slate-700 bg-slate-950/95 p-1.5 text-white shadow-xl backdrop-blur">
                  ...
                  <button type="button" onClick={exportAsPdf} aria-label="Exportar PDF" title="Exportar PDF" className="rounded-xl bg-cyan-400 p-2 text-slate-950"><Download className="h-4 w-4" /></button>
