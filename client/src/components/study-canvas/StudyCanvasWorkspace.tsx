...
import { getPaperVisualMetrics } from "./studyCanvasPaper";
...
export function StudyCanvasWorkspace({
  ...
  const canvasDimensions = useMemo(() => {
    ...
    if (paperSize === "a5") return { width: 840, height: 1188 };
    if (paperSize === "a3") return { width: 1680, height: 2376 };
    if (paperSize === "infinite") return { width: 2400, height: 4000 };
  ...
  const canvasWrapperClassName = fullscreen
    ...
    : compactMode
      ? "relative flex min-h-[calc(100dvh-150px)] items-start justify-center overflow-auto rounded-xl bg-slate-200/70 p-4 shadow-inner"
      : "rounded-2xl border border-slate-200 bg-white p-2 shadow-inner";

  const paperMetrics = getPaperVisualMetrics(paperSize);
  const paperVisualStyle = compactMode && !fullscreen ? {
    width: `min(${paperMetrics.maxWidth}px, 88vw)`,
    minHeight: undefined,
    aspectRatio: paperMetrics.aspectRatio ? String(paperMetrics.aspectRatio) : undefined,
  } : undefined;
        ...
        <div className={`${fullscreen ? "flex min-h-0 flex-1 flex-col gap-0 overflow-hidden bg-black p-0" : compactMode ? "h-full p-0" : "border-t border-violet-100 p-4 md:p-5"}`}>
          ...
          <div className={canvasWrapperClassName}>
            {compactMode && !fullscreen ? (
              <div className="absolute inset-x-2 top-2 z-30 mx-auto w-auto max-w-4xl">
                ...
                {compactPanel ? <div className="mt-2 rounded-2xl border border-slate-700 bg-slate-900/95 p-2.5 text-white shadow-xl backdrop-blur">
                  {compactPanel === "pages" ? <div className="flex flex-wrap items-center gap-2">
                    {pages.map((page, index) => <button key={page.id} type="button" onClick={() => { openPage(page.id); setCompactPanel(null); }} className={`h-10 min-w-10 rounded-xl px-3 text-xs font-black ${page.id === activePageId ? "bg-cyan-400 text-slate-950" : "bg-slate-800"}`}>P{index + 1}</button>)}
            ...
            {!loaded ? (
              ...
              <canvas
                ...
                style={{
                  ...(paperVisualStyle ?? {}),
                  aspectRatio: fullscreen ? undefined : paperVisualStyle?.aspectRatio ?? `${canvasDimensions.width} / ${canvasDimensions.height}`,
                  touchAction: "none",
                }}
              />
            )}
