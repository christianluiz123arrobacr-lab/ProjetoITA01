...
const SMART_CUTS: Array<{
...
}> = [
  {
    ...
    label: "Corte axial",
    description:
      "Passa pelo eixo principal e mostra a seção mais usada em cone/cilindro.",
  },
  {
    id: "base",
    label: "Corte da base",
...
function pointInsideSolid(type: SolidType, point: Vec3, sides: number) {
  ...
  if (type === "cylinder") {
    return (
      Math.abs(point.y) <= 1.225 && point.x ** 2 + point.z ** 2 <= 1.35 ** 2
    );
  }
...
function estimateSolidOverlap({
  ...
  const outerBounds = getSolidBounds(outerSolid, sides, normalizedOuterScale, {
    x: 0,
    y: 0,
    z: 0,
  });
      ...
      {[...transformedFaces]
        ...
        .map(face => (
          <path
            ...
            onDoubleClick={event =>
              handleElementDoubleClick(face.element, event)
            }
            onPointerDown={event => {
              event.preventDefault();
              event.stopPropagation();
      ...
      {[...transformedVertices]
        ...
        .map(vertex => (
          <circle
            ...
            onDoubleClick={event =>
              handleElementDoubleClick(vertex.element, event)
            }
            onPointerDown={event => {
              event.preventDefault();
              event.stopPropagation();
...
function getInscribedRelationship({
  ...
  if (outerSolid === "cylinder" && innerSolid === "regularPrism") {
    ...
    return {
      ...
      text: "A base do prisma fica inscrita no círculo da base do cilindro. Então o raio do cilindro é o raio circunscrito do polígono regular da base do prisma.",
    };
  }
  ...
  if (outerSolid === "cube" && innerSolid === "sphere") {
    return {
      ...
      text: "A esfera toca as seis faces do cubo. Por isso, o diâmetro da esfera é igual à aresta do cubo.",
    };
  }
  ...
  if (outerSolid === "sphere" && innerSolid === "cube") {
    ...
    return {
      ...
      text: "A diagonal espacial do cubo é igual ao diâmetro da esfera. Essa é a relação central do problema.",
    };
  }
  ...
  if (outerSolid === "cylinder" && innerSolid === "cone") {
    return {
      ...
      text: "Quando o cone tem a mesma base e a mesma altura do cilindro, seu volume é exatamente um terço do volume do cilindro.",
    };
  }
  ...
  return {
    ...
    text: "Neste modo, o sólido interno pode ser deslocado e escalado livremente. Para uma inscrição perfeita, normalmente ele precisa ficar centralizado e respeitar uma relação entre raio, aresta, altura ou diagonal.",
  };
}
...
function renderAxes({ angleX, angleY }: { angleX: number; angleY: number }) {
...
export function SpatialGeometrySimulator() {
  ...
  const [selectedAction, setSelectedAction] = useState<GeometryActionId | null>(
    null
  );
  ...
  const overlapResolution = isDraggingScene
    ? 18
  ...
  const unionVolume =
    mode === "inscribed"
      ? (overlapEstimate?.unionVolume ??
        outerMetrics.volume + innerMetrics.volume)
      : 0;
  ...
  const outerOnlyVolume =
    mode === "inscribed"
      ? (overlapEstimate?.outerOnlyVolume ?? outerMetrics.volume)
      : 0;
  ...
  const emptyVolume = mode === "inscribed" ? outerOnlyVolume : 0;
  ...
  const sceneObjects = [
    {
      ...
      label:
        mode === "simple"
          ...
          : `Externo: ${activeDefinition.label}`,
      solid: activeSolid,
  ...
  const selectedTargetLabel =
    selectedTarget === "inner" ? "interno" : "externo";
  ...
  const adaptiveInteractionTips = useMemo(() => {
    ...
    if (floatingMenu?.kind === "solid") {
      tips.push(
        "Use o menu aberto para ajustar base, altura, raio, cortes e fórmulas do sólido."
      );
    } else if (floatingMenu?.kind === "background") {
      tips.push(
        "Escolha um sólido ou uma cena pronta para montar uma comparação rapidamente."
      );
    } else if (measurementStart) {
      tips.push(
        `Origem: ${measurementStart.label}. Clique em outra face, aresta ou vértice para medir a distância.`
      );
    } else if (activeAdjustment) {
      tips.push(
        "Arraste o controle ou use os botões +/− para comparar como volume e área mudam."
      );
    } else if (mode === "inscribed" && selectedTarget === "inner") {
      tips.push(
        "Você está editando o sólido interno: mova, rotacione ou ajuste a escala para testar inscrição."
      );
    } else {
      tips.push(
        "Dê dois cliques no sólido para abrir o menu de edição; dê dois cliques no fundo para adicionar formas."
      );
    }

    if (mode === "inscribed" && !isCentered) {
      tips.push(
        "O sólido interno está deslocado; centralize para analisar uma inscrição clássica."
      );
    }

    if (mode === "inscribed" && exceedsSuggestedScale) {
      tips.push(
        "A escala interna passou do recomendado; reduza para evitar uma inscrição visualmente inválida."
      );
    }

    if (mode === "inscribed" && innerOutsideVolume > 0.01) {
      tips.push(
        `${formatNumber(innerOutsideVolume)} u³ do sólido interno estão fora do volume externo.`
      );
    }
  ...
  const propertyRows = [
    ...
    [
      "Área da base",
      inspectedMetrics.baseArea
        ? `${formatNumber(inspectedMetrics.baseArea)} u²`
        : "—",
    ],
    [
      "Área lateral",
      inspectedMetrics.lateralArea
        ? `${formatNumber(inspectedMetrics.lateralArea)} u²`
        : "—",
    ],
  ];
  ...
  const constraintRows = [
    ...
    {
      label: "Escala interna segura",
      ok: mode !== "inscribed" || !exceedsSuggestedScale,
    },
    {
      label: "Contido no externo",
      ok: mode !== "inscribed" || innerOutsideVolume <= 0.01,
    },
  ];
  ...
  function getClassicFitScale(outer: SolidType, inner: SolidType) {
    ...
    if (outer === "sphere" && inner === "cube") {
      return clamp(
        (2 * radius) / (Math.max(side, 0.1) * Math.sqrt(3)),
        0.2,
        1.05
      );
    }
  ...
  function applySmartCut(cut: SmartCutId) {
    ...
    if (cut === "central") {
      setSelectedAction(
        inspectedSolid === "sphere" ? "greatCircle" : "centralSection"
      );
      return;
    }
  ...
  const isAdjustingInner = mode === "inscribed" && selectedTarget === "inner";
  ...
  function nudgeAdjustment(target: AdjustmentTarget, delta: number) {
    const current = isAdjustingInner
      ? target === "base"
        ? innerBaseScale
        : target === "height"
          ? innerHeightScale
    ...
    setAdjustmentValue(target, current + delta);
  }
  ...
  function moveFloatingMenu(event: React.PointerEvent<HTMLDivElement>) {
    ...
    setFloatingMenu({
      ...floatingMenu,
      x: clamp(
        dragState.startMenuX + event.clientX - dragState.startX,
        12,
        maxX
      ),
      y: clamp(
        dragState.startMenuY + event.clientY - dragState.startY,
        12,
        maxY
      ),
    });
  ...
  function handlePointerMove(event: React.PointerEvent<HTMLDivElement>) {
    ...
    if (interactionMode === "rotateInner" && mode === "inscribed") {
      setInnerRotationY(
        normalizeAngle(dragState.startInnerRotationY + dx * 0.72)
      );
      setInnerRotationX(
        normalizeAngle(dragState.startInnerRotationX - dy * 0.72)
      );
      return;
    }

    setRotationY(clamp(dragState.startRotationY + dx * 0.42, -180, 180));
    setRotationX(clamp(dragState.startRotationX - dy * 0.28, -80, 80));
  }
  ...
  function renderFullscreenBackgroundMenu() {
    ...
    if (fullscreenMenuSection === "add") {
      return (
        <>
          {renderFullscreenMenuHeader(
            "Adicionar sólido",
            "Escolha a forma que entra como sólido interno."
          )}
            ...
            {QUICK_ADD_SOLIDS.map(solidType =>
              fullscreenMenuButton({
                label:
                  ...
                  solidType,
                description: SOLIDS.find(solid => solid.type === solidType)
                  ?.label,
                tone: "cyan",
    ...
    if (fullscreenMenuSection === "scenes") {
      return (
        <>
          {renderFullscreenMenuHeader(
            "Cenas prontas",
            "Abra uma situação clássica para explicar em aula."
          )}
          {renderFullscreenBackButton()}
    ...
    if (fullscreenMenuSection === "teach") {
      return (
        <>
          {renderFullscreenMenuHeader(
            "Ferramentas de aula",
            "Controle o que aparece durante a explicação."
          )}
          {renderFullscreenBackButton()}
          <div className="grid grid-cols-2 gap-2">
            {fullscreenMenuButton({
    ...
    return (
      <>
        {renderFullscreenMenuHeader(
          "Medidas rápidas",
          "Ative recursos de explicação matemática."
        )}
        {renderFullscreenBackButton()}
        <div className="grid grid-cols-2 gap-2">
          {fullscreenMenuButton({
  ...
  function renderFullscreenSolidMenu(target: SelectedTarget) {
    const targetLabel =
      target === "inner" ? "Sólido interno" : "Sólido externo";
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            {fullscreenMenuButton({
              label: "Mover / girar",
              description:
                target === "inner" ? "Posição e rotação" : "Girar cena",
              tone: "violet",
    ...
    if (fullscreenMenuSection === "transform") {
      return (
        <>
          {renderFullscreenMenuHeader(
            "Mover / girar",
            "Controle a manipulação direta do objeto."
          )}
          {renderFullscreenBackButton()}
          <div className="grid grid-cols-2 gap-2">
            {fullscreenMenuButton({
    ...
    if (fullscreenMenuSection === "formula") {
      return (
        <>
          {renderFullscreenMenuHeader(
            "Fórmulas e medidas",
            "Escolha o que destacar no desenho."
          )}
          {renderFullscreenBackButton()}
          <div className="grid grid-cols-2 gap-2">
            {fullscreenMenuButton({
    ...
    return (
      <>
        {renderFullscreenMenuHeader(
          "Visual do sólido",
          "Mostrar, ocultar, trocar ou encaixar."
        )}
        {renderFullscreenBackButton()}
        <div className="grid grid-cols-2 gap-2">
          {fullscreenMenuButton({
  ...
  function getCurrentGeometryElement(
    element: GeometryElement
  ): GeometryElement {
  ...
  function renderMeasurementStartMarker() {
    ...
    const point = projectPoint(
      currentStartElement.worldPoint,
      rotationX,
      rotationY
    );
        ...
        <div className="absolute left-3 right-3 top-3 z-40 flex flex-col gap-3 rounded-2xl border border-white/15 bg-white/90 px-3 py-3 shadow-2xl backdrop-blur lg:flex-row lg:items-center lg:justify-between lg:px-5">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              type="button"
              variant={
                interactionMode === "rotateInner" ? "default" : "outline"
              }
              onClick={() => {
                setMode("inscribed");
                setInteractionMode("rotateInner");
        ...
        >
          ...
          >
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {propertyRows.map(([label, value]) => (
                    <div
                      key={label}
                      className="rounded-xl bg-white/10 p-2 ring-1 ring-white/10"
                    >
                ...
                <div className="grid gap-2 text-xs">
                  {constraintRows.map(item => (
                    <div
                      key={item.label}
                      className="flex items-center justify-between rounded-xl bg-white/10 px-3 py-2 ring-1 ring-white/10"
                    >
            ...
            {mode === "inscribed" ? (
              <div className="absolute bottom-28 left-6 z-20 rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-white backdrop-blur">
                ...
                <p className="mt-1 text-xs font-semibold text-orange-100">
                  {overlapEstimate?.state ?? "calculando"} ·{" "}
                  {formatNumber(occupation)}% do externo
                </p>
              </div>
            ) : null}
            ...
            >
              ...
              {showAxes
                ? renderAxes({ angleX: rotationX, angleY: rotationY })
                : null}
              ...
              {activeSolid === "sphere"
                ...
                : renderMesh({
                    ...
                    onElementClick: element =>
                      handleElementClick(
                        buildGeometryElement({ ...element, target: "outer" })
                      ),
                    onElementDoubleClick: (element, event) =>
              ...
              {mode === "inscribed" && showInnerSolid
                ? innerSolid === "sphere"
                  ? renderSphere({
                      ...
                      onGeometryDoubleClick: event =>
                        openSolidMenu("inner", event),
                      onGeometryPointerDown: event =>
                        prepareSolidMenuOnTouch("inner", event),
                      theme: {
                        face: "#f97316",
                        edge:
                          selectedTarget === "inner" ? "#facc15" : "#fed7aa",
                  ...
                  : renderMesh({
                      ...
                      onElementClick: element =>
                        handleElementClick(
                          buildGeometryElement({ ...element, target: "inner" })
                        ),
                      onElementDoubleClick: (element, event) =>
                ...
                {isFullscreen ? (
                      ...
                      <div className="mt-3 rounded-2xl border border-cyan-300/20 bg-cyan-400/10 p-3">
                        ...
                        {isAdjustingInner ? (
                          <p className="mt-2 text-xs leading-5 text-cyan-100/80">
                            ...
                            independentes por objeto entram na próxima camada do
                            laboratório.
                          </p>
                        ) : null}
                      </div>
                            ...
                            <div className="mt-2 grid grid-cols-3 gap-2">
                              {[90, 180, 360].map(degrees => (
                                <button
                                  ...
                                  type="button"
                                  onClick={() =>
                                    rotateInnerBy(control.axis, degrees)
                                  }
                    ...
                    <div className="mt-3 border-t border-white/10 pt-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {QUICK_ADD_SOLIDS.map(solidType => (
                          ...
                          >
                            {
                              SOLIDS.find(solid => solid.type === solidType)
                                ?.shortLabel
                            }
                          </button>
                        ))}
                  ...
                  <>
                    ...
                    <p className="mt-3 text-xs leading-5 text-slate-300">
                      ...
                      “Mover sólido interno” para arrastar e ajustar a
                      inscrição.
                    </p>
                  </>
                )}
            ...
            <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  type="button"
                  variant={
                    interactionMode === "moveInner" ? "default" : "outline"
                  }
                  onClick={() => {
                    setMode("inscribed");
                    setInteractionMode("moveInner");
        ...
        >
          <Card className="border-slate-200 p-6">
            ...
            <p className="mt-2 text-sm leading-7 text-slate-600">
              Clique em um objeto para selecionar. Dê duplo clique no sólido no
              desenho para editar medidas ou trocar a forma.
            </p>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-slate-600 shadow-sm">
                      {
                        SOLIDS.find(solid => solid.type === object.solid)
                          ?.shortLabel
                      }
                    </span>
                  </div>
          ...
          <Card className="border-slate-200 p-6">
            ...
            {mode === "inscribed" ? (
              <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
                ...
                <div className="mt-4 rounded-2xl bg-white p-3">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-xs font-semibold text-emerald-900">
                        ...
                        ±
                        {formatNumber(
                          overlapEstimate?.estimatedErrorPercent ?? 0
                        )}
                        %
                      </p>
                    </div>
                  ...
                  <p className="mt-3 text-xs leading-5 text-emerald-800">
                    Enquanto você arrasta, o laboratório usa uma grade leve para
                    manter o movimento fluido. Ao soltar, ele recalcula conforme
                    o modo escolhido.
                  </p>
                </div>
              ...
              <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Escolha um encaixe clássico acima ou dê duplo clique/segure no
                  fundo do laboratório para adicionar uma forma.
                </p>
              </div>
            )}
          </Card>
          ...
          <Card className="border-slate-200 p-6">
            ...
            <p className="mt-2 text-sm leading-7 text-slate-600">
              Escolha um corte para revelar a figura plana que aparece dentro do
              sólido. É aqui que nascem muitas contas de diagonal, geratriz,
            ...
            <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-amber-900">
                ...
                círculo máximo; para cubo e paralelepípedo, a diagonal revela o
                triângulo retângulo interno.
              </p>
            </div>
          </Card>
          ...
          <Card className="border-slate-200 p-6">
            ...
            <p className="mt-2 text-sm leading-7 text-slate-600">
              A planificação mostra de onde vem a área total: bases mais faces
              laterais. É o antídoto contra decorar fórmula sem enxergar as
              peças.
            </p>
            ...
            {showNet ? (
              <div className="mt-5 rounded-2xl border border-fuchsia-200 bg-fuchsia-50 p-4">
                ...
                <div className="mt-4 rounded-2xl bg-white p-3">
                  ...
                  <MathFormula
                    formula={inspectedMetrics.formulas.area}
                    display={true}
                  />
                  <MathFormula
          ...
          <Card className="border-slate-200 p-6">
            ...
            <div className="mt-5 grid gap-4">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="mt-2">
                  <MathFormula
                    formula={outerMetrics.formulas.volume}
                    display={true}
                  />
                </div>
                <div className="mt-2">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="mt-2">
                  <MathFormula
                    formula={outerMetrics.formulas.area}
                    display={true}
                  />
                </div>
                <div className="mt-2">
              ...
              <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                {exceedsSuggestedScale ? (
                  <p className="mt-2 text-sm font-bold text-red-700">
                    A escala passou de 100%. O sólido interno provavelmente
                    atravessa o externo.
                  </p>
                ) : null}
              </div>
              ...
              <div className="mt-5 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    step="0.01"
                    value={
                      (innerBaseScale + innerHeightScale + innerRadiusScale) / 3
                    }
                    onChange={event =>
