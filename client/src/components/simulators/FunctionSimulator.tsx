...
const DEFAULT_VIEW = { centerX: 0, centerY: 0, scaleX: 48, scaleY: 48 };
...
function loadInitialState(): FunctionSimulatorStoredState {
  ...
  return (
    parseFunctionSimulatorStorage(window.localStorage.getItem(STORAGE_KEY)) ?? {
      functions: DEFAULT_FUNCTIONS,
      view: DEFAULT_VIEW,
      showGrid: true,
      equalScale: true,
...
export function FunctionSimulator() {
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (touchRef.current.size === 2 && pinchRef.current) {
      const points = Array.from(touchRef.current.values());
      ...
      setView(current => ({
        ...current,
        scaleX: Math.min(500, Math.max(8, pinchRef.current!.scaleX * factor)),
        scaleY: Math.min(
          500,
          Math.max(
            8,
            (equalScale ? pinchRef.current!.scaleX : pinchRef.current!.scaleY) *
              factor
          )
        ),
      }));
    ...
    if (dragRef.current)
      setView(current => ({
        ...
        centerX:
          ...
          (event.clientX - dragRef.current!.x) / current.scaleX,
        centerY:
          dragRef.current!.centerY +
          (event.clientY - dragRef.current!.y) / current.scaleY,
      }));
  };
  ...
  const clearAll = () => {
    ...
    if (filled > 1 && !window.confirm("Apagar todas as funções preenchidas?"))
      return;
    setFunctions(DEFAULT_FUNCTIONS.map(item => ({ ...item, expression: "" })));
    setSelectedIndex(0);
    setNotice("");
  };
      ...
      <div className="min-w-0 space-y-4">
        <div className="rounded-3xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            <Button variant="outline" onClick={() => setView(DEFAULT_VIEW)}>
