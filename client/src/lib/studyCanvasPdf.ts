...
export function buildStudyCanvasPdf(jpegPages: string[], title: string) {
  if (!jpegPages.length) throw new Error("Não há páginas para exportar.");
  const images = jpegPages.map(dataUrlBytes);
  ...
  images.forEach((image, index) => {
    ...
    objects[imageId - 1] = concat([encode(`<< /Type /XObject /Subtype /Image /Width 2400 /Height 1400 /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${image.length} >>\nstream\n`), image, encode("\nendstream")]);
    const stream = `q 842 0 0 491 0 52 cm /Im${index} Do Q`;
    objects[contentId - 1] = encode(`<< /Length ${stream.length} >>\nstream\n${stream}\nendstream`);
    objects[pageId - 1] = encode(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 842 595] /Resources << /XObject << /Im${index} ${imageId} 0 R >> >> /Contents ${contentId} 0 R >>`);
  });
...
export function safePdfFileName(title: string) { return `${title.trim().replace(/[^A-Za-zÀ-ÿ0-9 _-]+/g, "").replace(/\s+/g, "-").toLowerCase() || "caderno"}-projeto-vetor.pdf`; }
