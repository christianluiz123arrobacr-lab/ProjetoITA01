...
async function renderMathImage(formula: string, fontSize: number, color: string) {
  ...
  const pending = new Promise<HTMLImageElement | null>(resolve => {
    void (async () => {
      ...
      const ratio = viewBox.width > 0 && viewBox.height > 0 ? viewBox.width / viewBox.height : 1;
      const height = Math.max(fontSize + 8, Math.ceil(fontSize * 1.55));
      const width = Math.max(2, Math.ceil(height * ratio));
...
export async function generateQuestionPdf(input: { questions: Question[]; filterSummary: string; logoUrl?: string }) {
  ...
  for (let index = 0; index < input.questions.length; index += 1) {
    ...
    const estimatedHeight = 226 + Math.ceil(textualLength / 96) * 25
      + (question.imageUrl ? 230 : 0) + question.options.filter(option => option.imageUrl).length * 150;
