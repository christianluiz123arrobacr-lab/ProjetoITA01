...
import { buildQuestionRichContent, groupRichQuestionContent, parseRichQuestionText, serializeRichQuestionText } from "./richQuestionContent";
...
function svgAttributes(source: string) {
  return Object.fromEntries(Array.from(source.matchAll(/([\w-]+)="([^"]*)"/g), match => [match[1], match[2]]));
}

function averageHex(first: string, last: string) {
  const channels = [0, 2, 4].map(index => Math.round((Number.parseInt(first.slice(index + 1, index + 3), 16) + Number.parseInt(last.slice(index + 1, index + 3), 16)) / 2));
  return `#${channels.map(value => value.toString(16).padStart(2, "0")).join("")}`;
}

function svgPathToPdf(path: string) {
  const tokens = path.match(/[MmLlHhVvZz]|-?\d+(?:\.\d+)?/g) ?? []; let command = ""; let x = 0; let y = 0; let startX = 0; let startY = 0; const output: string[] = [];
  for (let index = 0; index < tokens.length;) {
    const token = tokens[index];
    if (/^[MmLlHhVvZz]$/.test(token)) { command = token; index += 1; if (/^[Zz]$/.test(command)) { output.push("h"); x = startX; y = startY; } continue; }
    if (command === "H" || command === "h") { x = command === "h" ? x + Number(token) : Number(token); index += 1; output.push(`${x} ${y} l`); continue; }
    if (command === "V" || command === "v") { y = command === "v" ? y + Number(token) : Number(token); index += 1; output.push(`${x} ${y} l`); continue; }
    const nextX = Number(token); const nextY = Number(tokens[index + 1]); index += 2; const relative = command === command.toLowerCase(); x = relative ? x + nextX : nextX; y = relative ? y + nextY : nextY;
    if (command === "M" || command === "m") { output.push(`${x} ${y} m`); startX = x; startY = y; command = command === "m" ? "l" : "L"; } else output.push(`${x} ${y} l`);
  }
  return output.join(" ");
}

/** Converts the real public brand SVG into a reusable vector PDF form. */
export function buildPdfLogoForm(svg: string) {
  if (!/<svg\b/.test(svg)) throw new Error("Logo SVG do Projeto Vetor inválido.");
  const gradients = new Map<string, string>();
  for (const match of Array.from(svg.matchAll(/<(?:linearGradient|radialGradient)\b([^>]*)>([\s\S]*?)<\/(?:linearGradient|radialGradient)>/g))) {
    const id = svgAttributes(match[1]).id; const colors = Array.from(match[2].matchAll(/stop-color="(#[0-9a-fA-F]{6})"/g)).map(item => item[1]);
    if (id && colors.length) gradients.set(id, averageHex(colors[0], colors.at(-1)!));
  }
  const color = (value = "#000000") => value.startsWith("url(#") ? gradients.get(value.slice(5, -1)) ?? "#000000" : value;
  const commands = ["q 1 0 0 -1 0 512 cm"];
  for (const match of Array.from(svg.matchAll(/<(circle|path|text)\b([^>]*?)(?:\/>|>([\s\S]*?)<\/\1>)/g))) {
    const [, tag, raw, body = ""] = match; const attrs = svgAttributes(raw); const fill = attrs.fill && attrs.fill !== "none" ? `${rgb(color(attrs.fill))} rg` : ""; const stroke = attrs.stroke && attrs.stroke !== "none" ? `${rgb(color(attrs.stroke))} RG ${Number(attrs["stroke-width"] || 1)} w` : "";
    if (tag === "circle") {
      const cx = Number(attrs.cx); const cy = Number(attrs.cy); const radius = Number(attrs.r); const k = radius * 0.5522847498;
      commands.push(`${fill} ${stroke} ${cx + radius} ${cy} m ${cx + radius} ${cy + k} ${cx + k} ${cy + radius} ${cx} ${cy + radius} c ${cx - k} ${cy + radius} ${cx - radius} ${cy + k} ${cx - radius} ${cy} c ${cx - radius} ${cy - k} ${cx - k} ${cy - radius} ${cx} ${cy - radius} c ${cx + k} ${cy - radius} ${cx + radius} ${cy - k} ${cx + radius} ${cy} c ${fill && stroke ? "B" : fill ? "f" : "S"}`);
    } else if (tag === "path") commands.push(`${fill} ${stroke} ${svgPathToPdf(attrs.d || "")} ${fill && stroke ? "B" : fill ? "f" : "S"}`);
    else {
      const size = Number(attrs["font-size"] || 12); const spacing = Number(attrs["letter-spacing"] || 0); const value = body.replace(/<[^>]+>/g, "").trim(); let x = Number(attrs.x || 0); if (attrs["text-anchor"] === "middle") x -= (textWidth(value, size, true) + Math.max(0, value.length - 1) * spacing) / 2;
      commands.push(`q 1 0 0 -1 0 0 cm BT /FB ${size} Tf ${spacing} Tc ${fill} 1 0 0 1 ${x} ${-Number(attrs.y || 0)} Tm ${pdfString(value)} Tj ET Q`);
    }
  }
...
export class VectorPdf {
  ...
  blob(subject: string) {
    ...
    put(8, `<< /Type /XObject /Subtype /Form /BBox [0 0 512 512] /Resources << /Font << /FB 5 0 R >> >> /Length ${enc.encode(this.logoForm).length} >>\nstream\n${this.logoForm}\nendstream`);
    const imageStart = 9;
...
export function splitPdfMathSegments(value: string, formulaOnly = false): PdfTextSegment[] {
  if (formulaOnly) return [{ kind: "math", value: normalizeMathSource(value).replace(/^\s*(?:\$\$?|\\\[)/, "").replace(/(?:\$\$?|\\\])\s*$/, ""), display: true }];
  return parseRichQuestionText(value).map(node => {
    if (node.type === "inlineMath") return { kind: "math", value: node.value, display: false };
    if (node.type === "blockMath") return { kind: "math", value: node.value, display: true };
    if (node.type === "lineBreak") return { kind: "text", value: "\n", display: false };
    if (node.type === "image") return { kind: "text", value: "", display: false };
    return { kind: "text", value: node.value, display: false };
  });
}
...
export async function generateQuestionPdf(input: { questions: Question[]; filterSummary: string; logoUrl?: string }) {
  ...
  for (let index = 0; index < input.questions.length; index += 1) {
    ...
    const statementHeight = groupRichQuestionContent(buildQuestionRichContent(question)).reduce((height, group) => height + (group.type === "image" ? QUESTION_PDF_LAYOUT.statementImageMaxHeight + 8 : estimatePdfRichTextHeight(serializeRichQuestionText(group.nodes), PAGE_WIDTH - MARGIN * 2)), 0);
    const alternativesHeight = question.options.reduce((height, option) => height + estimatePdfRichTextHeight(option.text ?? "", PAGE_WIDTH - MARGIN * 2 - 29, 10) + 16 + (option.imageUrl ? QUESTION_PDF_LAYOUT.optionImageMaxHeight + 8 : 0), 0);
    const minimumQuestionHeight = 43 + statementHeight + alternativesHeight + QUESTION_PDF_LAYOUT.resolutionSpaceHeight;
    ensure(Math.min(minimumQuestionHeight, CONTENT_BOTTOM - CONTENT_TOP)); pdf.text(`${index + 1}.`, MARGIN, y + 12, 12, true, NAVY); y += 20;
    ...
    for (const group of groupRichQuestionContent(buildQuestionRichContent(question))) {
      if (group.type === "image") await drawQuestionImage(group.url, QUESTION_PDF_LAYOUT.statementImageMaxWidth, QUESTION_PDF_LAYOUT.statementImageMaxHeight, MARGIN);
      else await richText(serializeRichQuestionText(group.nodes), MARGIN, PAGE_WIDTH - MARGIN * 2);
    }
    y += 7;
    for (let optionIndex = 0; optionIndex < question.options.length; optionIndex += 1) {
      const option = question.options[optionIndex]; const optionStart = y; const optionWidth = PAGE_WIDTH - MARGIN * 2 - 29; const estimated = option.text ? estimatePdfRichTextHeight(option.text, optionWidth, 10) + 10 : 20; const keepWithResolution = optionIndex === question.options.length - 1 ? QUESTION_PDF_LAYOUT.resolutionSpaceHeight : 0; ensure(Math.min(estimated + (option.imageUrl ? QUESTION_PDF_LAYOUT.optionImageMaxHeight + 8 : 0) + keepWithResolution, CONTENT_BOTTOM - CONTENT_TOP));
    ...
    if (!question.options.length) ensure(QUESTION_PDF_LAYOUT.resolutionSpaceHeight + 20);
    // The last alternative preflights this area, preventing an orphaned
    // "ESPAÇO PARA RESOLUÇÃO" at the top of a new page.
    pdf.text("ESPAÇO PARA RESOLUÇÃO", MARGIN, y + 10, 7, true, "#52677b"); y += QUESTION_PDF_LAYOUT.resolutionSpaceHeight;
  }
  footer();
