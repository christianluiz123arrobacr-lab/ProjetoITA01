...
describe("GET da página pública", () => {
  it("responde 200 para uma questão pública válida sem vazar gabarito", async () => {
    const handler = createPublicQuestionPageHandler(async () => ({
      slug: request.query.slug,
      title: "Questão de Física — EsPCEx 2025",
      institution: "EsPCEx",
      year: 2025,
      discipline: "Física",
      contents: ["Dinâmica"],
      subjects: [],
      difficulty: "Médio",
      statement: "Determine a força resultante.",
      statementAfterImage: "",
      formula: "",
      imageUrl: null,
      options: [{ key: "a", text: "10 N", imageUrl: null }],
      related: [],
    }));
    const response = responseMock();
    await handler(request as any, response as any);
    expect(response.statusCode).toBe(200);
    expect(response.body).toContain("Determine a força resultante");
    expect(response.body).not.toContain("alternativa_correta");
    expect(response.body).not.toContain("Resolução revisada");
  });
