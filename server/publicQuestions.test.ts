...
import { buildPublicSlugBase, chooseUniquePublicSlug, isPubliclyEligible, isValidPublicSlug, PUBLIC_SITE_ORIGIN, toInitialPublicQuestion } from "./publicQuestions";
...
describe("páginas públicas de questões", () => {
  ...
  it("sitemap contém somente origem canônica e entradas recebidas do filtro seguro", () => {
    ...
    expect(xml.match(/https:\/\/www\.projetovetor\.com/g)?.length).toBe(5);
  });
