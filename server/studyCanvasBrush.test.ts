...
const inkPipeline = readFileSync(
  new URL("../client/src/components/study-canvas/studyCanvasInk.ts", import.meta.url),
  "utf8"
);
...
describe("perfis visuais da escrita", () => {
  ...
  it("mantém pressão expressiva, pontas graduais e zoom interpolado", () => {
    expect(inkPipeline).toContain("const eased");
    expect(inkPipeline).not.toContain("previousPoint.width");
