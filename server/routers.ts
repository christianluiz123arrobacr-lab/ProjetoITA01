...
import { createGoogleDriveConnectUrl, createNotebook, disconnectGoogleDrive, getNotebook, googleDriveStatus, listNotebooks, renameNotebook, trashNotebook, updateNotebook } from "./googleDrive/googleDriveService.js";
...
export const appRouter = router({
  ...
  notebooks: router({
    list: protectedProcedure.query(({ ctx }) => listNotebooks(ctx.user.id)),
    create: protectedProcedure.input(z.object({ name: z.string().trim().min(1).max(80), paper: notebookPaperSchema })).mutation(({ ctx, input }) => createNotebook(ctx.user.id, input.name, input.paper)),
    get: protectedProcedure.input(z.object({ documentId: googleFileIdSchema })).query(({ ctx, input }) => getNotebook(ctx.user.id, input.documentId)),
    update: protectedProcedure.input(z.object({ documentId: googleFileIdSchema, document: notebookDocumentSchema, expectedModifiedTime: z.string().datetime(), force: z.boolean().default(false) })).mutation(({ ctx, input }) => updateNotebook(ctx.user.id, input.documentId, input.document, input.expectedModifiedTime, input.force)),
    rename: protectedProcedure.input(z.object({ documentId: googleFileIdSchema, name: z.string().trim().min(1).max(80) })).mutation(({ ctx, input }) => renameNotebook(ctx.user.id, input.documentId, input.name)),
    delete: protectedProcedure.input(z.object({ documentId: googleFileIdSchema })).mutation(({ ctx, input }) => trashNotebook(ctx.user.id, input.documentId)),
  }),
  system: systemRouter,
