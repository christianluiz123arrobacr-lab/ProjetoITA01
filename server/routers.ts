...
import { z } from "zod";
import { chemistryResolutionBlockSchema, safeChemistryLatexSchema, safeResolutionTextSchema } from "../shared/chemistryContent";
...
export const appRouter = router({
  ...
  questions: router({
    ...
    list: publicProcedure
      ...
      .query(async ({ input }) => {
        const rows = await fetchAllQuestionPages(async (from, to) => {
          let query = supabaseAdmin.from("questoes").select("*");

          if (input?.subject) query = query.eq("disciplina", input.subject);
          if (input?.exam) query = query.eq("banca", input.exam);
          if (input?.year) query = query.eq("ano", input.year);
          if (input?.difficulty) query = query.eq("dificuldade", input.difficulty);
          if (input?.institution) query = query.eq("instituição", input.institution);
        ...
        return rows.map(safeQuestionDto);
      }),
