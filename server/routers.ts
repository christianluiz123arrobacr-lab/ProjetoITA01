...
export const appRouter = router({
  ...
  notebooks: router({
    ...
    exportPdf: protectedProcedure.input(z.object({ documentId: googleFileIdSchema, name: z.string().trim().min(1).max(80), pdfBase64: z.string().max(16_000_000) })).mutation(({ ctx, input }) => uploadNotebookPdf(ctx.user.id, input.documentId, input.name, input.pdfBase64)),
