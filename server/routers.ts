...
import { normalizeVetText } from "../client/src/lib/vetEngine.js";
...
export const appRouter = router({
  ...
  vet: router({
    ...
    createMockSession: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        for (const target of desired) {
          ...
          const { data } = await supabaseAdmin.from("questoes").select("id,conteudos,conteudo,assunto").eq("publicada", true).overlaps("conteudos", contents.map((item: any) => item.conteudo)).limit(100);
          const candidates = [...(data ?? [])].sort((a: any, b: any) => Number(attemptedIds.has(a.id)) - Number(attemptedIds.has(b.id)) || stableVetOrder(session.id, a.id) - stableVetOrder(session.id, b.id));
          for (const row of candidates) {
            if (used.has(row.id)) continue;
            const content = contents.find((item: any) => (row.conteudos ?? [row.conteudo, row.assunto]).some((topic: string) => normalizeVetText(topic) === normalizeVetText(item.conteudo)));
            if (!content) continue;
        ...
        if (selected.length < targetTotal) {
          ...
          const { data: fallbackRows } = await supabaseAdmin.from("questoes").select("id,conteudos,conteudo,assunto").eq("publicada", true).overlaps("conteudos", fallbackContents.map((item: any) => item.conteudo)).limit(200);
          const candidates = [...(fallbackRows ?? [])].sort((a: any, b: any) => Number(attemptedIds.has(a.id)) - Number(attemptedIds.has(b.id)) || stableVetOrder(session.id, a.id) - stableVetOrder(session.id, b.id));
          for (const row of candidates) {
            if (used.has(row.id)) continue;
            const content = fallbackContents.find((item: any) => (row.conteudos ?? [row.conteudo, row.assunto]).some((topic: string) => normalizeVetText(topic) === normalizeVetText(item.conteudo)));
            if (!content) continue;
            used.add(row.id); selected.push({ row, block: content.block, content });
            if (selected.length >= targetTotal) break;
          }
        }
        if (selected.length) await supabaseAdmin.from("vet_mock_session_items").insert(selected.map((item, index) => ({ session_id: session.id, question_id: item.row.id, position: index + 1, strategic_content: item.content.conteudo, block: item.block, recommendation_score: item.content.priorityScore })));
        ...
        return { id: session.id, totalQuestions: selected.length };
      }),
    ...
    getMockResult: protectedProcedure.input(z.object({ sessionId: z.string().uuid() })).query(async ({ ctx, input }) => {
      const { data, error } = await supabaseAdmin.from("vet_mock_sessions").select("id,target_exam,focus_subject,mode,status,started_at,completed_at,total_questions,total_answered,correct_answers,accuracy,engine_version,engine_snapshot,created_at").eq("id", input.sessionId).eq("user_id", ctx.user.id).maybeSingle();
    ...
    refreshCollectiveStats: adminProcedure.mutation(async ({ ctx }) => {
      ...
      if (error) throw new TRPCError({ code: "BAD_REQUEST", message: error.message });
      return { updatedGroups: Number(data ?? 0) };
    }),
