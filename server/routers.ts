...
import { assertUserCanCheckoutPlan, hasLegacyFounderEligibility, isSamePlanFamily, publicPlanAvailability } from "./billing/legacyFounderPricing.js";
...
export const appRouter = router({
  ...
  admin: router({
    ...
    updateBillingPlan: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const { error } = await supabaseAdmin.rpc("admin_update_billing_plan", {
          p_admin_user_id: ctx.user.id,
          p_plan_id: input.planId,
          p_name: input.name,
          p_description: input.description ?? null,
          p_price_cents: input.priceCents,
          p_max_active_subscriptions: input.maxActiveSubscriptions,
          p_is_active: input.isActive,
          p_is_public: input.isPublic,
          p_display_order: input.displayOrder,
          p_requires_legacy_founder_eligibility: input.requiresLegacyFounderEligibility,
          p_eligibility_change_confirmation: input.eligibilityChangeConfirmation ?? null,
        });
        if (error) throw new TRPCError({ code: "BAD_REQUEST", message: error.message });
