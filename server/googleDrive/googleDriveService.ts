...
export async function uploadNotebookPdf(userId: string, notebookId: string, name: string, pdfBase64: string) {
  ...
  const upload = multipartPdf({
    name: `${safeNotebookName(name)}.pdf`,
    parents: metadata.parents,
    mimeType: "application/pdf",
    appProperties: { projetoVetorType: "notebook-export", sourceNotebookId: notebookId },
  }, bytes);
  const result = await driveFetch(token, "https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,webViewLink", {
    method: "POST", headers: { "content-type": upload.contentType }, body: upload.body,
  });
  return result.json() as Promise<{ id: string; webViewLink?: string }>;
}
