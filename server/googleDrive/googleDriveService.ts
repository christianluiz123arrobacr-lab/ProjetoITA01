...
async function driveFetch(token: string, url: string, init: RequestInit = {}) {
  ...
  if (!response.ok) {
    ...
    let reason = "unknown";
    try {
      const payload = await response.clone().json() as { error?: { status?: string; errors?: Array<{ reason?: string }> } };
...
export function isProjectVetorNotebookMetadata(metadata: {
  ...
  return (
    !metadata.trashed &&
    [NOTEBOOK_MIME, LEGACY_NOTEBOOK_MIME].includes(metadata.mimeType ?? "") &&
...
function multipartPdf(metadata: object, body: Buffer) {
  const boundary = `pv-${randomBytes(12).toString("hex")}`;
...
export function notebookPdfUploadTarget(pdfFileId?: string | null) {
  return pdfFileId
    ...
    : { method: "POST" as const, path: "https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,webViewLink" };
}
...
export async function uploadNotebookPdf(userId: string, notebookId: string, name: string, pdfBase64: string) {
  ...
  const upload = multipartPdf(pdfMetadata, bytes);
  ...
  if (!pdfFileId) {
    const linkedNotebook = await driveFetch(token, `https://www.googleapis.com/drive/v3/files/${encodeURIComponent(notebookId)}?fields=id,modifiedTime`, {
