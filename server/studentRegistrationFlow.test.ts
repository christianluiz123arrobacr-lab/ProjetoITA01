...
describe("cadastro automático do aluno", () => {
  ...
  it("preserva limites, papel student e rollback do perfil", () => {
    expect(registrationProcedure).toContain("await Promise.all([");
  ...
  it("usa extensão ESM no conteúdo químico carregado pelo router serverless", () => {
    expect(router).toContain('from "../shared/chemistryContent.js"');
    expect(router).not.toContain('from "../shared/chemistryContent"');
  });

  it("não mostra erros de parsing JSON internos para o aluno", () => {
    expect(registerPage).toContain("getRegistrationErrorMessage(error)");
    expect(registerPage).toContain("O servidor não conseguiu concluir seu cadastro");
    expect(registerPage).toContain("unexpected token|json|server error");
  });
