...
import { buildVectorPdfTestDocument, estimatePdfRichTextHeight, getPdfMathVectorSize, isSafePdfMathSvg, latexToPdfText, measurePdfMath, shouldRenderStandaloneFormula, splitPdfMathSegments, textWidth } from "../client/src/lib/questionPdfGenerator";
...
describe("exportação de questões em PDF", () => {
  ...
  it("mantém a marca d'água vetorial em todas as páginas", async () => {
    const source = new TextDecoder("latin1").decode(await buildVectorPdfTestDocument("Questao", 2));
    expect(source.match(/\/GS1 gs/g)).toHaveLength(2);
    expect(source.match(/\/Logo Do/g)).toHaveLength(2);
