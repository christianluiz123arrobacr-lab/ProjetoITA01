...
export const PUBLIC_QUESTION_SELECT = "id,codigo,disciplina,diciplina,conteudo,conteudos,assunto,assuntos,banca,ano,dificuldade,enunciado,enunciado_pos_imagem,url_imagem,formula,A,B,C,D,E,a_url_imagem,b_url_imagem,c_url_imagem,d_url_imagem,e_url_imagem,instituição,public_slug,is_public,public_noindex,public_published_at,publicada,alternativa_correta";
...
export function buildPublicSlugBase(row: PublicQuestionRow, publicCode: string): string {
  ...
  const subject = row.disciplina || row.diciplina || "estudos";
...
export function toInitialPublicQuestion(row: PublicQuestionRow, related: PublicQuestionRow[] = []) {
  ...
  const discipline = String(row.disciplina || row.diciplina || "Estudos");
  ...
  return {
    ...
    related: related.filter(isPubliclyEligible).slice(0, 3).map((item) => ({
      ...
      title: `Questão de ${item.disciplina || item.diciplina || "Estudos"} — ${item.instituição || item.banca || "Projeto Vetor"}${item.ano ? ` ${item.ano}` : ""}`,
...
async function loadEligibleRow(slug: string): Promise<PublicQuestionRow | null> {
  ...
  if (error) throw new Error("Não foi possível carregar a página pública.");
  return isPubliclyEligible(data) ? data : null;
}

export async function getPublicQuestion(slug: string) {
  const row = await loadEligibleRow(slug);
  if (!row) return null;
  const discipline = row.disciplina || row.diciplina;
  let query = supabaseAdmin.from("questoes").select(PUBLIC_QUESTION_SELECT)
    .eq("is_public", true).eq("public_noindex", false).eq("publicada", true).neq("public_slug", slug).limit(8);
  if (discipline) query = query.eq("disciplina", discipline);
  const { data } = await query;
  return toInitialPublicQuestion(row, (data ?? []).sort(() => Math.random() - 0.5));
}

export async function answerPublicQuestion(slug: string, selectedOption: string | null) {
  if (selectedOption !== null && !/^[a-e]$/.test(selectedOption)) throw new TRPCError({ code: "BAD_REQUEST", message: "Alternativa inválida." });
  const row = await loadEligibleRow(slug);
  ...
  return {
    ...
    correctOption,
    resolution: ((data ?? []) as ResolutionRow[]).map((block) => ({
