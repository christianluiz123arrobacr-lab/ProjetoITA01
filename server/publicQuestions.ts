...
import { formatDifficultyLabel } from "../shared/difficulty.js";
...
export function toInitialPublicQuestion(row: PublicQuestionRow, related: PublicQuestionRow[] = []) {
  ...
  return {
    ...
    institution,
    bank: row.banca ? String(row.banca).trim() : null,
    year: typeof row.ano === "number" ? row.ano : null,
    discipline,
    contents,
    subjects,
    difficulty: row.dificuldade ? formatDifficultyLabel(row.dificuldade) : null,
