...
describe("VET canônico", () => {
  ...
  it("frontend envia somente a resposta e backend usa RPC canônica", () => {
    ...
    expect(router).toMatch(/key: `quiz:attempt:\$\{ctx\.user\.id\}`,[\s\S]{0,500}allowInMemoryFallback: true/);
    expect(router).toMatch(/key: `question-report:\$\{ctx\.user\.id\}`,[\s\S]{0,500}allowInMemoryFallback: true/);
  ...
  it("listagem pública omite gabarito e resolução", () => {
    ...
    expect(router).toContain("return rows.map(safeQuestionDto)");
  });
