...
describe("proteção multitoque do editor compartilhado", () => {
  ...
  it("trava somente o dedo em navegação no modo Caderno", () => {
    expect(shouldNavigateInsteadOfDraw("touch", true, true)).toBe(true);
    expect(shouldNavigateInsteadOfDraw("pen", true, false)).toBe(false);
    expect(shouldNavigateInsteadOfDraw("mouse", true, false)).toBe(false);
  });
});
