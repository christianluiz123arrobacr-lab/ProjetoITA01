...
describe("produção de billing orchestration", () => {
  ...
  it("explica com segurança o conflito de assinatura ativa na reconciliação", async () => {
    const raw = new Error('duplicate key value violates unique constraint "idx_billing_subscriptions_one_active_per_user"');
    expect(mapPaymentReconciliationError(raw)).toBe(
      "Foi encontrada uma assinatura ativa anterior para este aluno. O pagamento precisa ser aplicado à assinatura principal.",
    );
  });

  it("audita o conflito de assinatura ativa com a mensagem compreensível", async () => {
    const local = { id: "11111111-1111-4111-8111-111111111111", subscription_id: "22222222-2222-4222-8222-222222222222", gateway: "mercadopago", gateway_payment_id: "9001", status: "pending" };
    const writeAuditLog = vi.fn().mockResolvedValue(undefined);
    await expect(reconcileMercadoPagoPaymentByAdmin({ billingPaymentId: local.id, adminUserId: "admin-1" }, {
      loadLocalPayment: vi.fn().mockResolvedValue(local),
      getGatewayPayment: vi.fn().mockResolvedValue({ id: 9001, status: "approved" }),
      processGatewayPayment: vi.fn().mockRejectedValue(new Error('duplicate key value violates unique constraint "idx_billing_subscriptions_one_active_per_user"')),
      reloadLocalPayment: vi.fn(),
      writeAuditLog,
    } as any)).rejects.toThrow("Foi encontrada uma assinatura ativa anterior");
    expect(writeAuditLog).toHaveBeenCalledWith(expect.objectContaining({
      level: "warning",
      metadata: expect.objectContaining({ error: expect.stringContaining("assinatura ativa anterior") }),
    }));
  });
