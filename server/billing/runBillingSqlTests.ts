...
-- An approved Mercado Pago payment must merge into an already active manual
-- subscription instead of violating the one-active-subscription invariant.
insert into public.profiles(id, email, nome) values
  ('00000000-0000-4000-8000-000000000103', 'manual-root@example.test', 'Manual Root')
on conflict (id) do nothing;
insert into public.billing_subscriptions(id, user_id, plan_id, status, gateway, current_period_start, current_period_end, metadata)
values
  ('00000000-0000-4000-8000-000000000310', '00000000-0000-4000-8000-000000000103', '00000000-0000-4000-8000-000000000201', 'active', 'manual', now(), now() + interval '15 days', '{"legacy":true}'),
  ('00000000-0000-4000-8000-000000000311', '00000000-0000-4000-8000-000000000103', '00000000-0000-4000-8000-000000000201', 'pending', 'mercadopago', null, null, '{"payment_method":"pix"}')
on conflict (id) do nothing;
insert into public.billing_payments(id, subscription_id, original_subscription_id, user_id, plan_id, gateway, gateway_payment_id, payment_method, status, amount_cents, currency, access_duration_value, access_duration_unit)
values ('00000000-0000-4000-8000-000000000410', '00000000-0000-4000-8000-000000000311', '00000000-0000-4000-8000-000000000311', '00000000-0000-4000-8000-000000000103', '00000000-0000-4000-8000-000000000201', 'mercadopago', 'mp-pix-manual-root', 'mercadopago_pix', 'pending', 1000, 'BRL', 30, 'days')
on conflict (id) do nothing;
select * from public.apply_approved_mercadopago_payment('00000000-0000-4000-8000-000000000410', 'mp-pix-manual-root', 'approved', null, now(), 30);
select is((select applied_to_subscription_id from public.billing_payments where id='00000000-0000-4000-8000-000000000410'), '00000000-0000-4000-8000-000000000310'::uuid, 'Pix usa assinatura manual ativa como raiz canônica');
select is((select status from public.billing_subscriptions where id='00000000-0000-4000-8000-000000000311'), 'expired', 'Origem pendente mesclada fica expirada');
select is((select count(*)::int from public.billing_subscriptions where user_id='00000000-0000-4000-8000-000000000103' and status='active'), 1, 'Usuário com raiz manual mantém somente uma assinatura ativa');
select ok((select metadata->>'merged_into_subscription_id' = '00000000-0000-4000-8000-000000000310' from public.billing_subscriptions where id='00000000-0000-4000-8000-000000000311'), 'Mesclagem da origem fica auditável');
select ok((select already_applied from public.apply_approved_mercadopago_payment('00000000-0000-4000-8000-000000000410', 'mp-pix-manual-root', 'approved', null, now(), 30)), 'Reconciliação em raiz manual é idempotente');
select is((select count(*)::int from public.billing_subscriptions where user_id='00000000-0000-4000-8000-000000000103' and status='active'), 1, 'Repetição não cria segunda assinatura ativa');
...
select * from public.reserve_mercadopago_recurring_checkout_slot(
  '00000000-0000-4000-8000-000000000101', 'mp-ledger@example.test', '00000000-0000-4000-8000-000000000201',
  '00000000-0000-4000-8000-000000000998', now()+interval '5 minutes', '{"payment_method":"card"}') \gset normal_
select ok(:'normal_should_create'::boolean, 'Checkout volta a ser permitido após reconciliação confirmada');
select public.release_mercadopago_checkout_reservation(
  :'normal_subscription_id'::uuid, 'normal release', null, null, null, null);
select ok((select status='failed' and not recurring_slot_active and gateway_reconciliation_status is null
  from public.billing_subscriptions where id=:'normal_subscription_id'::uuid),
  'Liberação normal encerra reserva sem reconciliação');

insert into public.profiles(id, email, nome) values
  ('00000000-0000-4000-8000-000000000102', 'mp-reconcile@example.test', 'MP Reconcile')
on conflict (id) do nothing;
select * from public.reserve_mercadopago_recurring_checkout_slot(
  '00000000-0000-4000-8000-000000000102', 'mp-reconcile@example.test', '00000000-0000-4000-8000-000000000201',
  '00000000-0000-4000-8000-000000000997', now()+interval '5 minutes', '{"payment_method":"card"}') \gset reconcile_
select public.release_mercadopago_checkout_reservation(
  :'reconcile_subscription_id'::uuid,
  'gateway created and compensation failed',
  'gateway_created_local_failed',
  'temporary cancellation failure',
  'preapproval-external-1',
  null
);
select ok((select recurring_state='reconciliation_required' and not recurring_slot_active
    and gateway_reconciliation_status='gateway_created_local_failed'
    and gateway_subscription_id='preapproval-external-1'
  from public.billing_subscriptions where id=:'reconcile_subscription_id'::uuid),
  'Falha compensatória persiste reconciliação com slot inativo e identificador externo');
select pg_temp.assert_reconciliation_blocks_slot(
...
select ok((select recurring_state='reconciliation_required' and not recurring_slot_active
    ...
    and gateway_subscription_id='preapproval-external-1'
  from public.billing_subscriptions where id=:'reconcile_subscription_id'::uuid),
  'Repetição preserva reconciliação e identificadores de forma idempotente');
