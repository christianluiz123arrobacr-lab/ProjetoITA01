...
import { assertUserCanCheckoutPlan } from "./legacyFounderPricing.js";
...
const ACTIVE_SUBSCRIPTION_CONFLICT_MESSAGE = "Foi encontrada uma assinatura ativa anterior para este aluno. O pagamento precisa ser aplicado à assinatura principal.";

export function mapPaymentReconciliationError(error: unknown) {
  const sanitized = sanitizeBillingError(error);
  if (
    sanitized.includes("idx_billing_subscriptions_one_active_per_user")
    || /duplicate key value violates unique constraint/i.test(sanitized)
  ) {
    return ACTIVE_SUBSCRIPTION_CONFLICT_MESSAGE;
  }
  return sanitized;
}
...
async function getPlanForCheckout(planSlug: string, userId: string): Promise<BillingPlanRow> {
  const slug = planSlug.trim();
  const { data, error } = await supabaseAdmin
    .from("billing_plans")
    .select("id, slug, name, description, price_cents, currency, billing_cycle, is_active, is_public, requires_legacy_founder_eligibility, max_active_subscriptions, invite_only")
    .eq("slug", slug)
    .maybeSingle();
  ...
  await assertUserCanCheckoutPlan(userId, data as BillingPlanRow);
...
export async function reconcileMercadoPagoPaymentByAdmin(
  ...
  try {
    ...
    const message = mapPaymentReconciliationError(error);
