-- Accept the historical quoted A-E option columns in canonical attempts.
-- Additive/idempotent; preserves all attempts and session protections.
...
begin
  ...
  if nullif(trim(coalesce(
       to_jsonb(v_question)->>lower(trim(p_selected_option)),
       to_jsonb(v_question)->>upper(trim(p_selected_option)),
       ''
     )), '') is null
